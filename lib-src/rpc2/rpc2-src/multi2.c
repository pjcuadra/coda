...
static int new_unpack(ARG *a_types, PARM **args, unsigned char **_ptr,
                      char *_end, long offset);
...
long MRPC_MakeMulti(int ServerOp, ARG ArgTypes[], RPC2_Integer HowMany,
                    ...
                    RPC2_Multicast *MCast, RPC2_HandleResult_func *HandleResult,
                    struct timeval *Timeout, ...)
{
    ...
    for (a_types = ArgTypes;; a_types++)
        if (a_types->mode == C_END) {
            (*a_types->endlog)(ServerOp, HowMany, CIDList,
                               RCList); /* Call Stub Log end */
            break;
        }
...
void pack(ARG *a_types, PARM **args, unsigned char **_ptr)
{
    ...
    case RPC2_STRUCT_TAG:
        say(0, RPC2_DebugLevel,
            "MakeMulti (pack): RPC2_STRUCT_TAG encountered\n");
        break;
        ...
        for (i = 0; i < maxiterate; i++) {
            for (field = (*a_types)->field; field->mode != C_END; field++) {
                if (field->type == RPC2_STRUCT_TAG)
                    ...
                    len += get_len(&field, strp, NO_MODE);
                switch (field->type) {
                case RPC2_BOUNDEDBS_TAG:
                    (*strp)++;
                case RPC2_COUNTEDBS_TAG:
                      ...
                      long rpcval, long offset)
{
    ...
    if (rpcval == RPC2_SUCCESS) {
        if (rspbuffer->Header.ReturnCode == RPC2_INVALIDOPCODE)
            ...
            for (a_types = ArgInfo->ArgTypes, args = ArgInfo->Args;
                 a_types->mode != C_END; a_types++) {
                switch (a_types->mode) {
                case MAX_BOUND:
                case IN_MODE:
                    args++;
                ...
                case IN_OUT_MODE:
                    if (a_types->type == RPC2_STRUCT_TAG) {
                        ...
                        ret = new_unpack(a_types, &args, &_ptr, _end, offset);
                    break;
                default:
                    assert(FALSE);
                }
...
int get_len(ARG **a_types, PARM **args, MODE mode)
{
    ...
    case RPC2_BYTE_TAG:
        if ((*a_types)->size != 0) {
            ...
            return (sizeof(
                int32_t)); /* don't set (*a_types)->size for single char variable */
    case RPC2_INTEGER_TAG:
    case RPC2_UNSIGNED_TAG:
    case RPC2_ENUM_TAG:
        return ((*a_types)->size);
    case RPC2_STRING_TAG:
        ...
        if (mode == IN_OUT_MODE)
            (*a_types)->size +=
                _PAD(strlen((char *)(*(*args)->stringp[0])) + 1);
        else
    ...
    default:
        say(0, RPC2_DebugLevel,
            "get_len: [can't happen]: impossible type tag: %d\n",
            (*a_types)->type);
        return (-1);
    }
}
                      ...
                      char *_end, long offset)
{
    ...
    case RPC2_ENCRYPTIONKEY_TAG:
        ...
        if (mode == IN_OUT_MODE) {
            ...
            memcpy(*((*args)->key), *_ptr, RPC2_KEYSIZE);
        _INCR(*_ptr, RPC2_KEYSIZE);
        _ROUND(*_ptr, sizeof(int32_t));
...
static void incr_struct_byte(ARG *a_types, PARM **args)
{
    *(char **)args += (a_types->bound) ? (a_types->bound) : 1;
    if (a_types[1].type == RPC2_BYTE_TAG)
        return;
    byte_pad(args);
}
