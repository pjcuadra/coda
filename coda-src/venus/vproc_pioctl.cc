...
void vproc::do_ioctl(VenusFid *fid, unsigned char nr, struct ViceIoctl *data)
{
        ...
        for (;;) {
            ...
            case _VIOC_ADD_MT_PT: {
                ...
                *arg = '\0';
                ...
                /* Do the operation. */
                f->PromoteLock();
                u.u_error = f->Symlink(contents, link_name, u.u_uid, 0644,
                                       FSDB->StdPri());
                if (u.u_error) {
                    break;
                }
    ...
    case _VIOC_GETFID: {
        ...
        int volmode = (nr == _VIOC_EXPANDOBJECT || nr == _VIOC_ENABLEREPAIR ||
                       nr == _VIOC_COLLAPSEOBJECT ||
                       nr == _VIOC_DISABLEREPAIR || nr == _VIOC_REPAIR) ?
                          VM_MUTATING :
                          VM_OBSERVING;
        ...
        for (;;) {
            ...
            u.u_error =
                FSDB->Get(&f, fid, u.u_uid, RC_STATUS, NULL, NULL, NULL, 1);
            if (u.u_error && u.u_error != EINCONS)
                goto OI_FreeLocks;
...
#undef endp
...

    {
...
#endif
        volent *v = 0;
        if ((u.u_error = VDB->Get(&v, MakeVolid(fid))))
            break;
        ...
        case _VIOCGETVOLSTAT: {
            ...
            uint64_t cml_bytes;
            int local_only = 0;
            if (data->in_size == sizeof(int) && *(int *)data->in == 1)
                local_only = 1;

            /* Retrieve the volume status from the server(s). */
            u.u_error = v->GetVolStat(&volstat, &Name, &conn_state, &age,
                                      &hogtime, &conflict, &cml_count,
                                      &cml_bytes, &OfflineMsg, &MOTD, u.u_uid,
                                      local_only);
            if (u.u_error)
                break;
            ...
            *(int32_t *)cp = (int32_t)conn_state;
            cp += sizeof(int32_t);
            *(int32_t *)cp = (int32_t)conflict;
            cp += sizeof(int32_t);
            *(int32_t *)cp = (int32_t)cml_count;
            cp += sizeof(int32_t);
            ...
            *(uint32_t *)cp = (uint32_t)age;
            cp += sizeof(uint32_t);
            *(uint32_t *)cp = (uint32_t)hogtime;
            cp += sizeof(uint32_t);
        ...
        case _VIOCSETVOLSTAT: {
            ...
            u.u_error =
                v->SetVolStat(&volstat, &Name, &OfflineMsg, &MOTD, u.u_uid);
            if (u.u_error)
                break;
        ...
        case _VIOCWHEREIS: {
            ...
            v->GetVids((VolumeId *)(data->out +
                                    (VSG_MEMBERS * sizeof(struct in_addr))));
            data->out_size =
                VSG_MEMBERS * (sizeof(struct in_addr) + sizeof(VolumeId));
            break;
        }
...
#undef agep
        ...
        case _VIOC_REP_CMD: {
            ...
            case REP_CMD_BEGIN: {
                ...
                rc = dir->Lookup(&localcache, fid, LOCALCACHE_HIDDEN, u.u_uid,
                                 CLU_CASE_SENSITIVE | CLU_TRAVERSE_MTPT, 1);
            ...
            case REP_CMD_LIST: {
                ...
                fprintf(temp_fp,
                        "%d entries total in the modify log for volume %s\n",
                        count, v->GetName());
                fflush(temp_fp);
    ...
    case _VIOC_ZLIMIT: {
        ...
        case _VIOC_LOOKASIDE: {
            ...
            LKParseAndExecute((char *)data->in, (char *)data->out,
                              CFS_PIOBUFSIZE - 1);
        ...
        case _VIOC_GET_MT_PT: {
            ...
            Realm *r = REALMDB->GetRealm((char *)data->in + sizeof(VolumeId));
            volid.Realm = r->Id();
            r->PutRef();
...
#undef hostids
        ...
        case _VIOC_GETPATH: {
            ...
            int out_size =
                CODA_MAXPATHLEN; /* needed since data->out_size is a short! */
            VenusFid vfid;
            MakeVenusFid(&vfid, realm->Id(), &fid);
...
#undef hostids
            if (u.u_error < 0)
                u.u_error = EINVAL; /* fail returns -1 */
            break;
        }
