		    ...
		    VenusFid *parent)
{
    ...
    if ((f = Find(key)) != NULL)
	{ f->print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("fsdb::Create: key found"); }
...
void fsdb::Put(fsobj **f_addr) {
	...
	{ f->print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("fsdb::Put: no locks!"); }
...
int fsdb::TranslateFid(VenusFid *OldFid, VenusFid *NewFid) 
{
	...
	if (Newf != 0) {
		f->print(LoggingSubsystem::GetInstance()->GetLogFile());
		Newf->print(LoggingSubsystem::GetInstance()->GetLogFile());
...
void fsdb::ReclaimFsos(int priority, int count) {
	...
	if (!REPLACEABLE(f))
	    { f->print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("fsdb::ReclaimFsos: !REPLACEABLE"); }
...
int fsdb::FreeBlockCount() {
	...
	while ((o = onext())) {
	    ...
	    if (f->flags.owrite == 0)
	    { f->print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("fsdb::FreeBlockCount: on owriteq && !owrite"); }
...
void fsdb::ReclaimBlocks(int priority, int nblocks) {
	...
	if (!REPLACEABLE(f))
	    { f->print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("fsdb::ReclaimBlocks: !REPLACEABLE"); }
