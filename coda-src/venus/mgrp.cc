...
{
    LOG(1,("mgrpent::~mgrpent %p, uid = %d, mid = %d, auth = %d\n",
           this, uid, McastInfo.Mgroup, authenticated));
	...
	MULTI_START_MESSAGE(ViceDisconnectFS_OP);
        int code = (int) MRPC_MakeMulti(ViceDisconnectFS_OP,
                                        ...
                                        rocc.handles, rocc.retcodes, rocc.MIp,
                                        0, 0);
...
void mgrpent::Put()
{
    LOG(100, ("mgrpent::Put %p, uid = %d, mid = %d, auth = %d, refcount = %d, detached = %d\n",
...
{
    LOG(100, ("mgrpent::KillMember: %p, uid = %d, mid = %d, host = %s, forcibly = %d\n",
