                                  ...
                                  int *mcode, int *rcode)
{
    ...
    *local = FSDB->Find(fid); /* find the object instead */
    ...
    if (rc || !(*global)) {
        LOG(0,
            ("CheckRepair_GetObjects: Couldn't find a global replica for fid (%s)!\n",
             FID_(fid)));
        return rc;
    }
...
static int CheckRepair_CheckAccess(fsobj *fso, int rights, char *msg,
                                   int *mcode, int *rcode)
{
    vproc *vp = VprocSelf();
    char path[MAXPATHLEN];
...
void cmlent::CheckRepair(char *msg, int *mcode, int *rcode)
{
    ...
    case CML_Store_OP:
        ...
        OBJ_ASSERT(this,
                   (ParentObj = LocalObjs[0]->pfso)); /*used to be global */
        ...
        *rcode = REPAIR_OVER_WRITE;
        rc =
            CheckRepair_CheckVVConflict(LocalObjs[0], GlobalObjs[0]->VV(), msg);
        break;
    ...
    case CML_Chown_OP:
        ...
        /* step 2 */
        OBJ_ASSERT(this, (ParentObj = LocalObjs[0]->pfso));
        rc = CheckRepair_CheckAccess(ParentObj, PRSFS_ADMINISTER, msg, mcode,
                                     rcode);
        if (rc)
            break;
    ...
    case CML_Create_OP:
        /* step 1 */
        rc = CheckRepair_GetObjects("create", &u.u_create.PFid, &GlobalObjs[0],
                                    &LocalObjs[0], msg, mcode, rcode);
        if (rc)
            break;

        /* step 2 */
        rc = CheckRepair_CheckAccess(GlobalObjs[0], PRSFS_INSERT, msg, mcode,
                                     rcode);
        if (rc)
            break;

        /* step 3 */
        rc = CheckRepair_CheckNameConflict(GlobalObjs[0], (char *)Name, 0, msg,
                                           mcode, rcode);
        break;
    ...
    case CML_Link_OP:
        ...
        rc = CheckRepair_GetObjects("link source", &u.u_link.PFid,
                                    &GlobalObjs[0], &LocalObjs[0], msg, mcode,
                                    rcode);
        if (rc)
            break;

        rc = CheckRepair_GetObjects("link target", &u.u_link.CFid,
                                    &GlobalObjs[1], &LocalObjs[1], msg, mcode,
                                    rcode);
        if (rc)
            break;
    ...
    case CML_Remove_OP:
        ...
        /* step 2 */
        rc = CheckRepair_CheckAccess(GlobalObjs[0], PRSFS_DELETE, msg, mcode,
                                     rcode);
        if (rc)
            break;
    ...
    case CML_Rename_OP:
        ...
        rc = CheckRepair_GetObjects("rename target object", &u.u_rename.SFid,
                                    &GlobalObjs[2], &LocalObjs[2], msg, mcode,
                                    rcode);
        if (rc)
            break;
        ...
        rc = CheckRepair_CheckAccess(GlobalObjs[1], PRSFS_INSERT, msg, mcode,
                                     rcode);
        if (rc)
            break;

        /* step 3 */
        rc = CheckRepair_CheckNameConflict(GlobalObjs[0], (char *)Name, 1, msg,
                                           mcode, rcode);
        if (rc)
            break;
...
int cmlent::DoRepair(char *msg, int rcode)
{
	    ...
	    */
...
#else /* new, uses individual VV's */
        {
            ...
            for (int i = 0; (i < VSG_MEMBERS) && !code; i++) {
                ...
                replicafid.Volume = volumeids[i];
                code = FSDB->Get(&(replicas[i]), &replicafid, vp->u.u_uid,
                                 RC_DATA);
                if (code || !replicas[i]) {
    ...
    case CML_Chmod_OP: {
        ...
        code = LObj->RepairSetAttr((unsigned long)-1, (Date_t)-1, (uid_t)-1,
                                   NewMode, (RPC2_CountedBS *)NULL);
        LObj->GetPath(LocalPath, 1);
        if (code == 0) {
    ...
    case CML_Chown_OP: {
        ...
        code = LObj->RepairSetAttr((unsigned long)-1, (Date_t)-1, NewOwner,
                                   (unsigned short)-1, (RPC2_CountedBS *)NULL);
        LObj->GetPath(LocalPath, 1);
        if (code == 0) {
    ...
    case CML_Create_OP: { /* do fid replacement after the creation succeeded. */
        fid  = &u.u_create.PFid;
        ...
        fsobj *target          = NULL;
        code =
            LPObj->RepairCreate(&target, (char *)Name, NewMode, FSDB->StdPri());
        LPObj->GetPath(LocalPath, 1);
        if (code == 0) {
            sprintf(msg, "create %s/%s succeeded", LocalPath, (char *)Name);
            target->UnLock(WR); /* release write lock on the new object */
        } else {
            sprintf(msg, "create %s/%s failed(%d)", LocalPath, (char *)Name,
                    code);
        }
        break;
    }
    case CML_Link_OP: {
        ...
        if (code == 0) {
            ...
            sprintf(msg, "link %s/%s failed(%d)", LocalPath, (char *)Name,
                    code);
        }
        break;
    }
    case CML_MakeDir_OP: {
        ...
        fsobj *target          = NULL;
        code =
            LPObj->RepairMkdir(&target, (char *)Name, NewMode, FSDB->StdPri());
        LPObj->GetPath(LocalPath, 1);
        if (code == 0) {
            sprintf(msg, "mkdir %s/%s succeeded", LocalPath, (char *)Name);
            target->UnLock(WR); /* release write lock on the new object */
        } else {
            sprintf(msg, "mkdir %s/%s failed(%d)", LocalPath, (char *)Name,
                    code);
        }
        break;
    }
    case CML_SymLink_OP: {
        ...
        unsigned short NewMode = LObj->stat.Mode;
        fsobj *target          = NULL;
        code = LPObj->RepairSymlink(&target, (char *)NewName, (char *)Name,
                                    NewMode, FSDB->StdPri());
        LPObj->GetPath(LocalPath, 1);
        if (code == 0) {
            sprintf(msg, "symlink %s/%s -> %s succeeded", LocalPath,
                    (char *)NewName, (char *)Name);
            target->UnLock(WR); /* relese write lock on the new object */
        } else {
            sprintf(msg, "symlink %s/%s -> %s failed(%d)", LocalPath,
                    (char *)NewName, (char *)Name, code);
        }
        break;
    }
    case CML_Remove_OP: {
        ...
        OBJ_ASSERT(this, GObj != NULL && (LObj == NULL || DYING(LObj) ||
                                          LObj->stat.LinkCount > 0));
        ...
        if (code == 0) {
            ...
            sprintf(msg, "remove %s/%s failed(%d)", LocalPath, (char *)Name,
                    code);
        }
        break;
    }
    case CML_RemoveDir_OP: {
        ...
        if (code == 0) {
            ...
            sprintf(msg, "rmdir %s/%s failed(%d)", LocalPath, (char *)Name,
                    code);
        }
        break;
    }
    case CML_Rename_OP: {
        ...
        OBJ_ASSERT(this,
                   GSPObj && LSPObj && GSPObj->IsDir() && LSPObj->IsDir());
        ...
        OBJ_ASSERT(this,
                   GTPObj && LTPObj && GTPObj->IsDir() && LTPObj->IsDir());
        ...
        LOG(100, ("cmlent::DoRepair: do rename on source object %s and %s\n",
                  FID_(&GObj->fid), FID_(&LObj->fid)));
        code = LTPObj->RepairRename(GSPObj, (char *)Name, GObj, (char *)NewName,
                                    (fsobj *)NULL);
        /*
        ...
        if (code == 0) {
            sprintf(msg, "rename %s/%s -> %s/%s succeeded", SPath, (char *)Name,
                    TPath, (char *)NewName);
        } else {
            sprintf(msg, "rename %s/%s -> %s/%s failed(%d)", SPath,
                    (char *)Name, TPath, (char *)NewName, code);
        }
        break;
    }
    case CML_Repair_OP: {
...
void cmlent::GetLocalOpMsg(char *msg)
{
    ...
    case CML_Rename_OP: {
        ...
        sprintf(msg, "rename %s/%s -> %s/%s", path, (char *)Name, tppath,
                (char *)NewName);
        break;
    }
    case CML_Repair_OP: {
        ...
        LOG(0,
            ("cmlent::GetLocalOpMsg: Disconnected Repair on %s\n", FID_(fid)));
