...
volent::~volent()
{
    ...
    if (refcnt != 0)
	{ print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("volent::~volent: non-zero refcnt"); }
}
...
int volent::Enter(int mode, uid_t uid)
{
		...
		if (IsReadWrite()) {
		    ...
		    if (rv->GetCML()->Owner() == UNSET_UID) {
                        if (mutator_count != 0 || rv->GetCML()->count() != 0 ||
                            rv->IsReintegrating())
			    { print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("volent::Enter: mutating, CML owner == %d\n", rv->GetCML()->Owner()); }
	...
	default:
	    print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("volent::Enter: bogus mode %d", mode);
    }
...
void volent::Exit(int mode, uid_t uid)
{
	...
	case VM_MUTATING:
		...
		{ print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("volent::Exit: mutating"); }
	...
	case VM_OBSERVING:
		...
		{ print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("volent::Exit: observing"); }
	...
	case VM_RESOLVING:
		...
		{ print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("volent::Exit: resolving"); }
	...
	default:
	    print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("volent::Exit: bogus mode %d", mode);
    }
...
volrep::~volrep()
{
	...
	{ print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("volrep::~volrep: htab remove"); }
...
repvol::~repvol()
{
	...
	{ print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("repvol::~repvol: htab remove"); }
