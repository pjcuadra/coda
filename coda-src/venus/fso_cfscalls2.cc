...
void fsobj::Release(int writep)
{
    ...
    if (!openers) {
        ...
        currc = active_read_segments.pop();
        /* Remove all active read segments in case there are some left 
         * behind */
        while (currc.isValid()) {
            LOG(0, ("fsobj::Release: Warning read segments [%d, %d] is still active (%s)\n",
