...
int repvol::FlushCOP2(mgrpent *m, RPC2_CountedBS *PiggyBS)
{
	...
	GetCOP2(&BS);
	if (BS.SeqLen == 0)
	    { print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("volent::FlushCOP2(Piggy): No Entries!\n"); }
...
void repvol::ClearCOP2(RPC2_CountedBS *BS)
{
	...
	if (c) {
		...
		{ print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("volent::ClearCOP2: remove"); }
	    delete c;
	}
    }
    BS->SeqLen = 0;
