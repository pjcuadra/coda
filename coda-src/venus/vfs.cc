...
#include <venus/vol.h> // FIXME: This is only needed for Begin_VFS

/* ***** VFS Operations  ***** */

int vfs::root(struct venus_cnode *vpp)
{
    LOG(1, "vfs::root\n");

    /* Set OUT parameter. */
    MAKE_CNODE2(*vpp, vproc::rootfid, C_VDIR);

    return 0;
}

int vfs::statfs(struct coda_statfs *sfs)
{
    LOG(1, "vfs::statfs\n");
...
int vfs::vget(struct venus_cnode *vpp, VenusFid *vfid, int what)
{
    int ret = 0;
    LOG(1, "vfs::vget: fid = %s, nc = %x\n", FID_(vfid),
        VprocSelf()->getUserContext().u_nc);

    VenusFid fid = *vfid;
    fsobj *f     = 0;

    if (VprocSelf()->getUserContext().u_nc && Logging::GetLogLevel() >= 100)
        VprocSelf()->getUserContext().u_nc->print(GetLogFile());

    /* return early if we're called to prefetch data */
    worker *w = (worker *)VprocSelf();
    if (w->type == VPT_Worker && (what & RC_DATA)) {
    ...
    for (;;) {
        VprocSelf()->Begin_VFS(&fid, CODA_VGET);
        if (ret)
            break;

        ret = FSDB->Get(&f, &fid, VprocSelf()->getUserContext().u_uid, what);
        if (ret) {
            if (ret == EINCONS) {
                ret = 0;
    ...
    FreeLocks:
        ...
        if (ret && VprocSelf()->getUserContext().u_nc)
            VprocSelf()->getUserContext().u_nc->CheckComponent(f);
        FSDB->Put(&f);
        int retry_call = 0;
        VprocSelf()->End_VFS(&retry_call);
        if (!retry_call)
            break;
    }
...
int vfs::open(struct venus_cnode *cp, int flags)
{
    int ret = 0;

    LOG(1, "vfs::open: fid = %s , flags = %x\n", FID_(&cp->c_fid), flags);
    ...
    for (;;) {
        VprocSelf()->Begin_VFS(&cp->c_fid, CODA_OPEN,
                               writep ? VM_MUTATING : VM_OBSERVING);
        if (ret)
            break;

        /* Get the object. */
        ret = FSDB->Get(&f, &cp->c_fid, VprocSelf()->getUserContext().u_uid,
                        RC_DATA);
        if (ret)
            goto FreeLocks;
        ...
        if (readp) {
            ...
            ret        = f->Access(rights, C_A_R_OK,
                            VprocSelf()->getUserContext().u_uid);
            if (ret)
                goto FreeLocks;
        }
        if (writep || truncp) {
	    ...
	    */
            ...
            ret = f->Access(rights, modes, VprocSelf()->getUserContext().u_uid);
            if (ret)
                goto FreeLocks;
        }

        /* Do the operation. */
        ret = f->Open(writep, truncp, cp, VprocSelf()->getUserContext().u_uid);
        if (ret)
            goto FreeLocks;
...
int vfs::close(struct venus_cnode *cp, int flags)
{
    int ret = 0;

    LOG(1, "vfs::close: fid = %s, flags = %x\n", FID_(&cp->c_fid), flags);
    ...
    VprocSelf()->Begin_VFS(&cp->c_fid, CODA_CLOSE,
                           writep ? VM_MUTATING : VM_OBSERVING);
    if (ret)
        goto Exit;
    ...
    if (!DYING(f) && !HAVEALLDATA(f) && !ISVASTRO(f))
        LOG(0,
            "vfs::close: Don't have DATA and not DYING! "
            "(fid = %s, flags = %x)\n",
            FID_(&cp->c_fid), flags);

    /* Do the operation. */
    ret = f->Close(writep,
                   VprocSelf()->getUserContext().u_uid /*, not_written */);

FreeLocks:
    FSDB->Put(&f);
    VprocSelf()->End_VFS(NULL);
...
int vfs::ioctl(struct venus_cnode *cp, unsigned char nr, struct ViceIoctl *data,
               int flags)
{
    int ret = 0;
    LOG(1, "vfs::ioctl(%d): fid = %s, com = %s\n",
        VprocSelf()->getUserContext().u_uid, FID_(&cp->c_fid), IoctlOpStr(nr));

    VprocSelf()->do_ioctl(&cp->c_fid, nr, data);
...
int vfs::getattr(struct venus_cnode *cp, struct coda_vattr *vap)
{
    int ret = 0;
    LOG(1, "vfs::getattr: fid = %s\n", FID_(&cp->c_fid));

    fsobj *f = 0;

    for (;;) {
        VprocSelf()->Begin_VFS(&cp->c_fid, CODA_GETATTR);
        if (ret)
            break;
...
/* The only attributes that are allowed to be changed are:
      ...
      va_mtime	    utimes
*/
int vfs::setattr(struct venus_cnode *cp, struct coda_vattr *vap)
{
    int ret = 0;
    LOG(1, "vfs::setattr: fid = %s\n", FID_(&cp->c_fid));
    ...
    for (;;) {
        VprocSelf()->Begin_VFS(&cp->c_fid, CODA_SETATTR);
        if (ret)
            break;
	 ...
	 * data as well. */
        volent *v = 0;
        ret       = VDB->Get(&v, MakeVolid(&cp->c_fid));
        if (ret)
            goto FreeLocks;
        if (v->IsReachable())
            rcrights |= RC_DATA;
		 ...
		 * when the file is closed... */
                ...
                ret = f->Access(PRSFS_ADMINISTER, C_A_F_OK,
                                VprocSelf()->getUserContext().u_uid);
                if (ret)
                    goto FreeLocks;
            }
            ...
            if (vap->va_size != VA_IGNORE_SIZE) {
                ...
                ret = f->Access(PRSFS_WRITE, C_A_W_OK,
                                VprocSelf()->getUserContext().u_uid);
                if (ret)
                    goto FreeLocks;
            }
            ...
            if (vap->va_mtime.tv_sec != VA_IGNORE_TIME1) {
                ret = f->Access(PRSFS_WRITE, C_A_F_OK,
                                VprocSelf()->getUserContext().u_uid);
                if (ret)
                    goto FreeLocks;
            }
        ...
        /* Do the operation. */
        f->PromoteLock();
        ret = f->SetAttr(vap, VprocSelf()->getUserContext().u_uid);
...
int vfs::access(struct venus_cnode *cp, int mode)
{
    int ret = 0;
    LOG(1, "vfs::access: fid = %s, mode = %#o\n", FID_(&cp->c_fid), mode);
    ...
    for (;;) {
        VprocSelf()->Begin_VFS(&cp->c_fid, CODA_ACCESS);
        if (ret)
            break;
    ...
    if (ret == EINCONS) {
        ret = (mode == R_OK || mode == F_OK) ? 0 : ENOENT;
        k_Purge(&cp->c_fid, 1);
    }
...
int vfs::lookup(struct venus_cnode *dcp, const char *name,
                struct venus_cnode *cp, int flags)
{
    int ret = 0;
    LOG(1, "vfs::lookup: fid = %s, name = %s, nc = %x\n", FID_(&dcp->c_fid),
        name, VprocSelf()->getUserContext().u_nc);

    fsobj *parent_fso = 0;
    fsobj *target_fso = 0;

    for (;;) {
        VprocSelf()->Begin_VFS(&dcp->c_fid, CODA_LOOKUP);
        if (ret)
            break;

        /* Get the parent object and verify that it is a directory. */
        ret = FSDB->Get(&parent_fso, &dcp->c_fid,
                        VprocSelf()->getUserContext().u_uid, RC_DATA);
        if (ret)
            goto FreeLocks;
        if (!parent_fso->IsDir()) {
            ret = ENOTDIR;
        ...
        if (STREQ(name, ".")) {
            ...
            if (parent_fso->IsRoot() && parent_fso->u.mtpoint)
                ret = FSDB->Get(&target_fso, &parent_fso->u.mtpoint->pfid,
                                VprocSelf()->getUserContext().u_uid, RC_DATA);
            else {
            ...
            VenusFid inc_fid;
            ret = parent_fso->Lookup(&target_fso, &inc_fid, name,
                                     VprocSelf()->getUserContext().u_uid,
    ...
    FreeLocks:
        /* Update namectxt if applicable. */
        if (ret == 0 && VprocSelf()->getUserContext().u_nc)
            VprocSelf()->getUserContext().u_nc->CheckComponent(target_fso);
        FSDB->Put(&parent_fso);
        FSDB->Put(&target_fso);
        int retry_call = 0;
        VprocSelf()->End_VFS(&retry_call);
...
int vfs::create(struct venus_cnode *dcp, char *name, struct coda_vattr *vap,
                int excl, int flags, struct venus_cnode *cp)
{
    int ret = 0;
    LOG(1, "vfs::create: fid = %s, name = %s, excl = %d, flags = %d\n",
    ...
    for (;;) {
        ...
        VprocSelf()->Begin_VFS(&dcp->c_fid, CODA_CREATE);
        if (ret)
            break;
        ...
        ret = parent_fso->Lookup(&target_fso, NULL, name,
                                 VprocSelf()->getUserContext().u_uid, flags);
        if (ret == 0) {
            ...
            if (readp) {
                ret = target_fso->Access(PRSFS_READ, C_A_R_OK,
                                         VprocSelf()->getUserContext().u_uid);
                if (ret)
                    goto FreeLocks;
            }
            if (writep || truncp) {
                ret = target_fso->Access(PRSFS_WRITE, C_A_W_OK,
                                         VprocSelf()->getUserContext().u_uid);
                if (ret)
                    goto FreeLocks;
            }
            ...
            FSDB->Put(&target_fso);
            ret = FSDB->Get(&target_fso, &target_fid,
                            VprocSelf()->getUserContext().u_uid, RC_DATA);
            if (ret)
                goto FreeLocks;
            ...
            ret = parent_fso->Access(PRSFS_INSERT, C_A_F_OK,
                                     VprocSelf()->getUserContext().u_uid);
            if (ret)
                goto FreeLocks;
            ...
            ret = parent_fso->Create(name, &target_fso,
                                     VprocSelf()->getUserContext().u_uid,
...
int vfs::remove(struct venus_cnode *dcp, char *name)
{
    int ret = 0;
    LOG(1, "vfs::remove: fid = %s, name = %s\n", FID_(&dcp->c_fid), name);
    ...
    for (;;) {
        VprocSelf()->Begin_VFS(&dcp->c_fid, CODA_REMOVE);
        if (ret)
            break;
        ...
        ret = parent_fso->Access(PRSFS_DELETE, C_A_F_OK,
                                 VprocSelf()->getUserContext().u_uid);
        if (ret)
            goto FreeLocks;
        ...
        parent_fso->PromoteLock();
        target_fso->PromoteLock();
        ret = parent_fso->Remove(name, target_fso,
                                 VprocSelf()->getUserContext().u_uid);
...
int vfs::link(struct venus_cnode *scp, struct venus_cnode *dcp, char *toname)
{
    int ret = 0;
    LOG(1, "vfs::link: fid = %s, td_fid = %s, toname = %s\n", FID_(&scp->c_fid),
        FID_(&dcp->c_fid), toname);
    ...
    for (;;) {
        VprocSelf()->Begin_VFS(&dcp->c_fid, CODA_LINK);
        if (ret)
            break;
	 ...
	 * data as well. */
        ...
        volent *v        = 0;
        ret              = VDB->Get(&v, MakeVolid(&scp->c_fid));
        if (ret)
            goto FreeLocks;
        if (v->IsReachable())
        ...
        if (FID_LT(scp->c_fid, dcp->c_fid)) {
            ret = FSDB->Get(&source_fso, &scp->c_fid,
                            VprocSelf()->getUserContext().u_uid, src_rcrights);
            if (ret)
                goto FreeLocks;
        ...
        ret = parent_fso->Lookup(&target_fso, NULL, toname,
                                 VprocSelf()->getUserContext().u_uid,
                                 CLU_CASE_SENSITIVE);
        if (ret == 0) {
            ret = EEXIST;
        ...
        ret = source_fso->Access(PRSFS_WRITE, C_A_F_OK,
                                 VprocSelf()->getUserContext().u_uid);
        if (ret)
            goto FreeLocks;
        ...
        ret = parent_fso->Link(toname, source_fso,
                               VprocSelf()->getUserContext().u_uid);
...
int vfs::rename(struct venus_cnode *spcp, char *name, struct venus_cnode *tpcp,
                char *toname)
{
    int ret = 0;
    LOG(1, "vfs::rename: fid = %s, td_fid = %s, name = %s, toname = %s\n",
    ...
    for (;;) {
        VprocSelf()->Begin_VFS(&spcp->c_fid, CODA_RENAME);
        if (ret)
            break;

        /* Acquire the parent(s). */
        if (SameParent) {
            ret = FSDB->Get(&t_parent_fso, &tpcp->c_fid,
                            VprocSelf()->getUserContext().u_uid, RC_DATA);
            if (ret)
                goto FreeLocks;
        } else {
            if (FID_LT(spcp->c_fid, tpcp->c_fid)) {
                ret = FSDB->Get(&s_parent_fso, &spcp->c_fid,
                                VprocSelf()->getUserContext().u_uid, RC_DATA);
                if (ret)
                    goto FreeLocks;
                ret = FSDB->Get(&t_parent_fso, &tpcp->c_fid,
        ...
        /* Verify that we have the necessary permissions in the parent(s). */
        {
            ...
            ret      = f->Access(PRSFS_DELETE, C_A_F_OK,
                            VprocSelf()->getUserContext().u_uid);
            if (ret)
                goto FreeLocks;
        }
        ret = t_parent_fso->Access(PRSFS_INSERT, C_A_F_OK,
                                   VprocSelf()->getUserContext().u_uid);
        if (ret)
            goto FreeLocks;
	 ...
	 */
        ...
        ret = t_parent_fso->Lookup(&t_fso, NULL, toname,
                                   VprocSelf()->getUserContext().u_uid,
                                   CLU_CASE_SENSITIVE);
        if (ret) {
            if (ret != ENOENT)
        ...
        if (TargetExists) {
            ...
            if (!SameParent) {
                ret = t_parent_fso->Access(PRSFS_DELETE, C_A_F_OK,
                                           VprocSelf()->getUserContext().u_uid);
                if (ret)
                    goto FreeLocks;
            }
            ...
            if (!SameParent && s_fso->IsDir() && !t_parent_fso->IsRoot()) {
                ...
                for (;;) {
                    ...
                    fsobj *test_fso = 0;
                    ret             = FSDB->Get(&test_fso, &test_fid,
                                    VprocSelf()->getUserContext().u_uid,
                                    RC_DATA);
                    if (ret)
                        goto FreeLocks;
	 ...
	 */
        ...
        ret = t_parent_fso->Rename(s_parent_fso, name, s_fso, toname, t_fso,
                                   VprocSelf()->getUserContext().u_uid);
...
int vfs::mkdir(struct venus_cnode *dcp, char *name, struct coda_vattr *vap,
               struct venus_cnode *cp)
{
    int ret = 0;
    LOG(1, "vfs::mkdir: fid = %s, name = %s\n", FID_(&dcp->c_fid), name);
    ...
    for (;;) {
        VprocSelf()->Begin_VFS(&dcp->c_fid, CODA_MKDIR);
        if (ret)
            break;
        ...
        ret = parent_fso->Mkdir(name, &target_fso,
                                VprocSelf()->getUserContext().u_uid,
                                vap->va_mode & 0777, FSDB->StdPri());
        if (ret)
            goto FreeLocks;
...
int vfs::rmdir(struct venus_cnode *dcp, char *name)
{
    int ret = 0;
    LOG(1, "vfs::rmdir: fid = %s, name = %s\n", FID_(&dcp->c_fid), name);
    ...
    for (;;) {
        VprocSelf()->Begin_VFS(&dcp->c_fid, CODA_RMDIR);
        if (ret)
            break;
        ...
        if (target_fso->IsRoot()) {
            ...
            CHOKE("vfs::rmdir: target is root");
        }
	 ...
	 * cfs rmmount */
        ...
        FSDB->Put(&target_fso);
        ret = FSDB->Get(&target_fso, &target_fid,
                        VprocSelf()->getUserContext().u_uid, RC_DATA, name);
        if (ret)
            goto FreeLocks;
        ...
        parent_fso->PromoteLock();
        target_fso->PromoteLock();
        ret = parent_fso->Rmdir(name, target_fso,
                                VprocSelf()->getUserContext().u_uid);
...
int vfs::symlink(struct venus_cnode *dcp, char *contents,
                 struct coda_vattr *vap, char *name)
{
    int ret = 0;
    LOG(1, "vfs::symlink: fid = (%s), contents = %s, name = %s\n",
    ...
    for (;;) {
        VprocSelf()->Begin_VFS(&dcp->c_fid, CODA_SYMLINK);
        if (ret)
            break;
        ...
        /* Do the operation. */
        parent_fso->PromoteLock();
        ret = parent_fso->Symlink(contents, name,
                                  VprocSelf()->getUserContext().u_uid, 0777,
                                  FSDB->StdPri());
        if (ret)
            goto FreeLocks;
...
int vfs::readlink(struct venus_cnode *cp, struct coda_string *string)
{
    int ret = 0;
    LOG(1, "vfs::readlink: fid = %s\n", FID_(&cp->c_fid));
    ...
    for (;;) {
        VprocSelf()->Begin_VFS(&cp->c_fid, CODA_READLINK);
        if (ret)
            break;
        ...
        /*
	 ret = f->Access(PRSFS_LOOKUP, C_A_F_OK, VprocSelf()->getUserContext().u_uid);
	 if (ret)
	     { if (ret == EINCONS) ret = ENOENT; goto FreeLocks; }
*/

        /* Retrieve the link contents from the cache. */
        ret = f->Readlink(buf, len - 1, &string->cs_len,
                          VprocSelf()->getUserContext().u_uid);
        if (ret)
            goto FreeLocks;
...
int vfs::fsync(struct venus_cnode *cp)
{
    int ret = 0;
    LOG(1, "vfs::fsync: fid = %s\n", FID_(&cp->c_fid));

    fsobj *f = 0;

    for (;;) {
        VprocSelf()->Begin_VFS(&cp->c_fid, CODA_FSYNC);
        if (ret)
            break;
...
int vfs::read(struct venus_cnode *node, uint64_t pos, int64_t count)
{
    int ret = 0;
    LOG(1, "vfs::read: fid = %s, pos = %d, count = %d\n", FID_(&node->c_fid),
        pos, count);

    fsobj *f = NULL;

    VprocSelf()->Begin_VFS(&node->c_fid, CODA_ACCESS_INTENT, VM_OBSERVING);
    ...
    ret = f->ReadIntent(VprocSelf()->getUserContext().u_uid,
                        VprocSelf()->getUserContext().u_priority, pos, count);
...
int vfs::write(struct venus_cnode *node, uint64_t pos, int64_t count)
{
    int ret = 0;
    LOG(1, "vfs::write: fid = %s, pos = %d, count = %d\n", FID_(&node->c_fid),
        pos, count);

    fsobj *f = NULL;

    VprocSelf()->Begin_VFS(&node->c_fid, CODA_ACCESS_INTENT, VM_MUTATING);
...
int vfs::read_finish(struct venus_cnode *node, uint64_t pos, int64_t count)
{
    int ret = 0;
    LOG(1, "vfs::read_finish: fid = %s, pos = %d, count = %d\n",
    ...
    ret = f->ReadIntentFinish(pos, count);
...
int vfs::write_finish(struct venus_cnode *node, uint64_t pos, int64_t count)
{
    int ret = 0;
    LOG(1, "vfs::write_finish: fid = %s, pos = %d, count = %d\n",
        FID_(&node->c_fid), pos, count);
    ...
    f->RemoveActiveSegment(pos, count);
...
int vfs::mmap(struct venus_cnode *node, uint64_t pos, int64_t count)
{
    int ret = 0;
    LOG(1, "vfs::mmap: fid = %s, pos = %d, count = %d\n", FID_(&node->c_fid),
        pos, count);
...
int vfs::verifyname(char *name, int flags)
{
    int length;
    ...
    if ((flags & NAME_NO_EXPANSION) && vproc::expansion(name) != NULL) {
        return EINVAL;
    }
