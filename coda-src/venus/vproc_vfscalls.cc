...
void vproc::vget(struct venus_cnode *vpp, VenusFid *vfid, int what)
{
    ...
    if (u.u_nc && LoggingSubsystem::GetInstance()->GetLoggingLevel() >= 100)
	u.u_nc->print(LoggingSubsystem::GetInstance()->GetLogFile());
...
void vproc::rmdir(struct venus_cnode *dcp, char *name) 
{
	...
	if (target_fso->IsRoot())
	    { target_fso->print(LoggingSubsystem::GetInstance()->GetLogFile()); CHOKE("vproc::rmdir: target is root"); }
