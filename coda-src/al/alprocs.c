...
int AL_htonAlist(INOUT AL_AccessList *Al)
{
    ...
    if (htonl(i) == i)
        return (0); /* host order == byte order */
...
int AL_NewCPS(IN int MinNoOfEntries, OUT PRS_InternalCPS **ICPS)
{
    ...
    *ICPS = (PRS_InternalCPS *)malloc(sizeof(PRS_InternalCPS) +
                                      (MinNoOfEntries - 1) * sizeof(int));
    if (*ICPS == NULL) {
...
int AL_Externalize(IN AL_AccessList *Alist, OUT AL_ExternalAccessList *Elist)
{
    ...
    if (Alist->PlusEntriesInUse + Alist->MinusEntriesInUse > AL_MaxExtEntries)
        return (-1);
    AL_NewExternalAlist(Alist->PlusEntriesInUse + Alist->MinusEntriesInUse,
                        Elist);
    nextc = *Elist;
...
int AL_Initialize(IN const char *Version)
{
    ...
    LogMsg(4, AL_DebugLevel, stdout,
           "Library version: '%s'\tHeader version: '%s'\t", AL_VERSION,
           Version);
...
int AL_GetInternalCPS(IN int Id, OUT PRS_InternalCPS **ICPS)
{
    ...
    LogMsg(1, AL_DebugLevel, stdout, "in AL_GetInternalCPS(%d, 0x%x)", Id,
           ICPS);
    ...
    if (((*ICPS)->InclEntries =
             pdb_array_to_array((*ICPS)->IdList, &(profile.cps))) < 0) {
        AL_FreeCPS(ICPS);
        PDB_freeProfile(&profile);
        return -2;
    }
...
void AL_PrintAlist(IN AL_AccessList *A)
{
    ...
    printf(
        "TotalNoOfEntries = %d  PlusEntriesInUse = %d  MinusEntriesInUse = %d\n",
