...
void usage(char *name)
{
    ...
    fprintf(
        stderr,
        "       [-dump <file> [skip <volid1> <volid2>...] | -load <file>]\n");
}
...
static int ReadGlobalState(int fd)
{
    ...
    if (norton_debug)
        printf("Read maximum vol ID: %08x\n", maxvolid);

    rvmlib_begin_transaction(restore);
    VSetMaxVolumeId(maxvolid);
    rvmlib_end_transaction(flush, &(status));
    if (norton_debug)
        printf("Set max vol id to %08x.  RVM Status = %d\n", maxvolid, status);
    return 1;
}
...
#define VNODESIZE(vclass) \
    ((vclass) == vLarge ? SIZEOF_LARGEDISKVNODE : SIZEOF_SMALLDISKVNODE)
                         ...
                         VnodeClass vclass)
{
    ...
    int nvnodes, vnode_index, vnode_num;
    PDirInode inode;
    int npages;
    ...
    while ((vnode_index = vnext(vnode)) != -1) {
        ...
        if (norton_debug)
            printf("Writing vnode number 0x%x\n", vnode_num);
        ...
        if (vclass == vLarge) {
            ...
            for (npages = 0; inode->di_pages[npages]; npages++) {
                if (write(fd, (void *)inode->di_pages[npages], DIR_PAGESIZE) ==
                    -1) {
...
{
    if (!DumpVnodeList(fd, vol, vol_index, vLarge))
        return 0;
    if (!DumpVnodeList(fd, vol, vol_index, vSmall))
        return 0;
    return 1;
}
    ...
    *newinode = (PDirInode)rvmlib_rec_malloc(sizeof(struct DirInode));
    ...
    for (int i = 0; i < DIR_MAXPAGES; i++)
        if (oldinode->di_pages[i]) {
            ...
            rvmlib_modify_bytes(shadowInode.di_pages[i], oldinode->di_pages[i],
                                DIR_PAGESIZE);
        }
...
static int ReadVnodeList(int fd, Volume *vp, VnodeClass vclass, int ResOn)
{
    ...
    while (nvnodes--) {
        ...
        if (norton_debug)
            printf("Reading vnode number %08x\n", vnode_num);
        ...
        if (vclass == vLarge) {
            ...
            for (i = 0; i < npages; i++) {
                ...
                if (!inode->di_pages[i]) {
                    fprintf(stderr,
                            "Unable to allocate a new directory page.\n");
                    rvmlib_abort(VFAIL);
                    return 0;
                }
...
static int ReadVolVnodes(int fd, Volume *vp, int ResOn)
{
    if (!ReadVnodeList(fd, vp, vLarge, ResOn))
        return 0;
    if (!ReadVnodeList(fd, vp, vSmall, ResOn))
        return 0;
    return 1;
}
...
static int HasBackVols(VolumeId *skipvols, int nskipvols)
{
    ...
    for (i = 0; (i < maxid) && (i < MAXVOLS); i++) {
        ...
        if (vol->data.volumeInfo->type == BACKVOL) {
            ...
            if (InSkipVolumeList(vol->data.volumeInfo->parentId, skipvols,
                                 nskipvols)) {
                continue;
            } else {
                ret = TRUE;
...
static int dump_server_state(char *dump_file, VolumeId *skipvols, int nskipvols)
{
    ...
    if (HasBackVols(skipvols, nskipvols)) {
        fprintf(
            stderr,
            "This server has backup volumes. They must be purged before proceeding.\n");
        return 0;
    }
    ...
    for (i = 0; (i < maxid) && (i < MAXVOLS); i++) {
        ...
        if ((vol->data.volumeInfo->type == BACKVOL) &&
            InSkipVolumeList(vol->data.volumeInfo->parentId, skipvols,
                             nskipvols)) {
            continue;
        }
