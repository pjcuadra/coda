...
*/
long FS_ViceGetVolumeInfo(RPC2_Handle RPCid, RPC2_String VolName,
                          VolumeInfo *Info)
{
    ...
    SLog(
        2,
        "ViceGetVolumeInfo returns %s, Volume %u, type %x, servers %x %x %x %x...",
        ViceErrorMsg(errorCode), Info->Vid, Info->Type, Info->Server0,
        Info->Server1, Info->Server2, Info->Server3);
    return (errorCode);
}
...
errout:
    SLog(2, "ViceGetVolumeLocation: Volume %08x, location %s, err %s", Vid,
         addr ? addr : "", ViceErrorMsg(err));
    return (err);
}
                            ...
                            RPC2_Unsigned IsReplicated)
{
    ...
    CODA_ASSERT(GetRights(client->CPS, aCL, aCLSize, (Rights *)&rights,
                          (Rights *)&anyrights) == 0);
...
Final:
    ...
    if (!errorCode) {
        SLog(5, "Name = %s, Motd = %s, offMsg = %s", name->SeqBody,
             motd->SeqBody, offlineMsg->SeqBody);
                            ...
                            RPC2_CountedBS *PiggyCOP2)
{
    ...
    SLog(
        5,
        "Min = %d Max = %d, Name = %d.%d %s, Offline Msg = %d.%d.%s, motd = %d.%d.%s",
        status->MinQuota, status->MaxQuota, name->MaxSeqLen, name->SeqLen,
        name->SeqBody, offlineMsg->MaxSeqLen, offlineMsg->SeqLen,
        offlineMsg->SeqBody, motd->MaxSeqLen, motd->SeqLen, motd->SeqBody);
    ...
    if ((errorCode = GetVolObj(vid, &volptr, VOL_EXCL_LOCK, 0, -1))) {
        SLog(0, "Error locking volume in ViceSetVolumeStatus: %s",
                    ...
                    RPC2_Integer *useconds)
{
    ...
    *useconds = tpl.tv_usec;
    ...
    SLog(1, "ViceGetTime for user %s at %s:%d on conn %d.", client->UserName,
         inet_ntoa(client->VenusId->host), ntohs(client->VenusId->port), RPCid);
    ...
    SLog(2, "GetTime returns %d, %d, errorCode %d", *seconds, *useconds,
         errorCode);
                          ...
                          RPC2_Integer et, RPC2_Integer at, RPC2_CountedBS *cid)
{
    ...
    if (errorCode) {
        SLog(0,
             "New connection setup FAILED, RPCid %d, security level %d, "
...
static void SetVolumeStatus(VolumeStatus *status, RPC2_BoundedBS *name,
                            RPC2_BoundedBS *offMsg, RPC2_BoundedBS *motd,
                            Volume *volptr)
{
    ...
    strncpy((char *)offMsg->SeqBody, V_offlineMessage(volptr),
            (int)name->MaxSeqLen);
...
static void SetViceStats(ViceStatistics *stats)
{
    ...
    stats->TotalFetches =
        Counters[GETATTRPLUSSHA] + Counters[GETACL] + Counters[FETCH];
    ...
    if (kd_opened == 0) {
        ...
        if (i != 0) {
            LogMsg(0, SrvDebugLevel, stdout,
                   "Could not nlist symbols from kernel: %s", kvm_geterr(kd));
            kd_opened = -1;
            return;
        }
        kd_opened = 1;
...
#define KVM_READ(kd, offset, buf, size) \
    (kvm_read(kd, (u_long)RawStats[offset].n_value, buf, size) != size)

    if (KVM_READ(kd, CPTIME, busy, sizeof busy)) {
        LogMsg(0, SrvDebugLevel, stdout,
               "Could not read CPTIME data from kernel: %s", kvm_geterr(kd));
        return;
    }
    ...
    if (KVM_READ(kd, BOOT, &bootTime, sizeof(bootTime))) {
        LogMsg(0, SrvDebugLevel, stdout,
               "Could not read BOOT data from kernel: %s", kvm_geterr(kd));
        return;
    }
    ...
    if (KVM_READ(kd, HZ, &hz, sizeof(hz))) {
        LogMsg(0, SrvDebugLevel, stdout,
               "Could not read HZ data from kernel: %s", kvm_geterr(kd));
        return;
    }
    stats->Spare4 = hz;
    ...
    if (KVM_READ(kd, DISK, &dk_ndrive, sizeof(dk_ndrive))) {
        LogMsg(0, SrvDebugLevel, stdout,
               "Could not read DISK data from kernel: %s", kvm_geterr(kd));
        return;
    }
}
#endif
...
#define PARSELINE(file, pattern, args...)           \
    do {                                            \
        int i;                                      \
        for (i = 0; i < 16; i++) {                  \
            if (fscanf(file, pattern, ##args) != 0) \
                break;                              \
...
static void SetSystemStats(ViceStatistics *stats)
{
    ...
    stats->UsrTime =
        (resource.ru_utime.tv_sec * 100 + resource.ru_utime.tv_usec / 10000);
    stats->SysTime =
        (resource.ru_stime.tv_sec * 100 + resource.ru_stime.tv_usec / 10000);
