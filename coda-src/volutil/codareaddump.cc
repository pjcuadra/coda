...
command_t list[] = { { "openDumpFile", OpenDumpFile, 0, "" }, /* FileName */
                     ...
                     { "showVolumeDiskData", showVolumeDiskData, 0,
                       "" }, /* no args */
...
static void showHeader(int largc, char **largv)
{
    ...
    printf("%s Dump Version = %d\n",
           (head.Incremental) ? "Incremental" : "Full", head.version);
...
static void showVolumeDiskData(int largc, char **largv)
{
    ...
    if (data.stamp.magic != 0) {
        ...
        printf(
            ...
            data.parentId, data.groupId, data.cloneId, data.backupId,
            data.restoredFromId);
        printf(
            "\tneedsCallback = %u\n\tdestroyMe = %u\n\tdontSalvage = %u\n\treserveb3 = %u\n",
            data.needsCallback, data.destroyMe, data.dontSalvage,
            data.reserveb3);
    ...
    printf(
        "\tmaxquota = %d\n\tminquota = %d\n\tmaxfiles = %d\n\tacctNum = %u\n\towner = %u\n",
        data.maxquota, data.minquota, data.maxfiles, data.accountNumber,
        data.owner);
    printf("\t");
    for (i = 0; i < 3; i++) {
    ...
    printf(
        "\tfilecount = %d\n\tlinkcount = %u\n\tdiskused = %d\n\tdayUse = %d\n\tdayUseDate = %u\n",
        data.filecount, data.linkcount, data.diskused, data.dayUse,
        data.dayUseDate);
    printf("\t");
    for (i = 0; i < 3; i++) {
    ...
    printf(
        "\tcreationDate = %u\n\taccessDate = %u\n\tupdateDate = %u\n\texpirationDate = %u\n",
        data.creationDate, data.accessDate, data.updateDate,
        data.expirationDate);
    printf("\tbackupDate = %u\n\tcopyDate = %u\n", data.backupDate,
           data.copyDate);
    printf("\t");
...
static void setIndex(int largc, char **largv)
{
    ...
    if ((strcmp(vnodeType, "large") != 0) &&
        (strcmp(vnodeType, "small") != 0)) {
