...
static int GetString(FILE *stream, char *to, unsigned int max)
{
    ...
    if (len > max) { /* Ensure we only use max room */
        LogMsg(0, VolDebugLevel, stdout,
               "GetString: String longer than max (%d>%d) truncating.", len,
               max);
        tail = len - max + 1;
        len  = max - 1;
    } else if (len == max) {
...
long WriteDump(RPC2_Handle _cid, RPC2_Unsigned offset, RPC2_Unsigned *nbytes,
               RPC2_Unsigned volid, SE_Descriptor *sed)
{
...
dumpstream::dumpstream(char *filename)
{
    ...
    if ((filename == NULL) || (filename[0] == 0)) {
        ...
        if (stream == NULL) {
            LogMsg(0, VolDebugLevel, stderr, "Can't open dump file %s",
                   filename);
            exit(EXIT_FAILURE);
        }
...
int dumpstream::getDumpHeader(struct DumpHeader *hp)
{
    ...
    if (fgetc(stream) != D_DUMPHEADER || !GetInt32(stream, &beginMagic) ||
    ...
    while ((tag = fgetc(stream)) > D_MAX) {
        ...
        case 'i':
            ...
            break;
        case 'I':
            if (!GetInt32(stream, (unsigned int *)&hp->oldest) ||
                !GetInt32(stream, (unsigned int *)&hp->latest))
                return 0;
            break;
        }
    }
...
int dumpstream::getVolDiskData(VolumeDiskData *vol)
{
    ...
    if (fgetc(stream) != D_VOLUMEDISKDATA) {
        LogMsg(0, VolDebugLevel, stdout,
               "Volume header missing from dump %s!\n", name);
    ...
    while ((tag = fgetc(stream)) > D_MAX && tag != EOF) {
        ...
        case 'B':
            ...
            break;
        case 'O':
            GetString(stream, vol->offlineMessage,
                      (int)sizeof(vol->offlineMessage));
            break;
        case 'M':
...
int dumpstream::EndOfDump()
{
    ...
    if (magic != DUMPENDMAGIC) {
        LogMsg(0, VolDebugLevel, stderr, "Dump Magic Value Incorrect for %s",
               name);
        return -1;
    }

    if (fgetc(stream) != EOF) {
        LogMsg(0, VolDebugLevel, stderr, "Unrecognized postamble in dump %s",
               name);
        return -1;
    }
...
int dumpstream::getVnodeIndex(VnodeClass Type, long *nVnodes, long *listsize)
{
    ...
    if (Type == vLarge) {
        ...
        if (fgetc(stream) != D_LARGEINDEX) {
            LogMsg(0, VolDebugLevel, stderr,
                   "ERROR: Large Index not found in %s", name);
            return -1;
        }
    } else if (Type == vSmall) {
        IndexType = vSmall;
        if (fgetc(stream) != D_SMALLINDEX) {
            LogMsg(0, VolDebugLevel, stderr,
                   "ERROR: Small Index not found in %s", name);
            return -1;
        }
    } else {
        LogMsg(0, VolDebugLevel, stderr,
               "Illegal type passed to GetVnodeIndex!");
        return -1;
    }
    ...
    while ((tag = fgetc(stream)) > D_MAX && tag != EOF) {
        ...
        default:
            LogMsg(0, VolDebugLevel, stderr,
                   "Unexpected field of Vnode found in %s.", name);
            exit(EXIT_FAILURE);
        }
    }
...
int dumpstream::skip_vnode_garbage()
{
    ...
    if (tag == D_DIRPAGES) {
        ...
        LogMsg(10, VolDebugLevel, stdout,
               "SkipVnodeData: Skipping dirpages for %s", name);
        ...
        for (unsigned int i = 0; i < npages; i++) { /* Skip directory pages */
            ...
            if (tag != 'P') {
                LogMsg(0, VolDebugLevel, stderr,
                       "Restore: Dir page does not have a P tag");
                return -1;
            }
        ...
        LogMsg(10, VolDebugLevel, stdout,
               "SkipVnodeData: Skipping file data for %s", name);

        if (!GetInt32(stream, &filesize))
            return -1;

        for (nbytes = filesize; nbytes;
             nbytes -= size) { /* Skip the filedata */
            if (nbytes < size)
                size = nbytes;
            if (fread(buf, size, 1, stream) != 1) {
                LogMsg(0, VolDebugLevel, stderr,
                       "Error reading file from dump %s", name);
                return -1;
            }
        }
    } else {
...
int dumpstream::readDirectory(PDirInode *dip)
{
    ...
    if (IndexType != vLarge) {
        LogMsg(
            ...
            "dumpstream::readDirectory() called when IndexType is %d instead of vLarge",
            IndexType);
        return (-1);
    }
    ...
    if (nexttag != D_DIRPAGES) {
        LogMsg(
            0, VolDebugLevel, stderr,
            "dumpstream::readDirectory()found tag %c when expecting D_DIRPAGES",
            nexttag);
        return (-1);
    }
    ...
    *dip = (DirInode *)malloc(sizeof(DirInode));
    ...
    for (unsigned int i = 0; i < npages; i++) {
        ...
        if (nexttag != 'P') {
            LogMsg(0, VolDebugLevel, stderr,
                   "dumpstream::readDirectory: Dir page does not have a P tag");
            return -1;
        }
        if (!GetByteString(stream, (byte *)(*dip)->di_pages[i], DIR_PAGESIZE)) {
            LogMsg(
                0, VolDebugLevel, stderr,
                "dumpstream::readDirectory: read of dir page #%d of %d pages failed",
                i, npages);
            return -1;
        }
    }
                         ...
                         VnodeDiskObject *vdo)
{
    ...
    LogMsg(10, VolDebugLevel, stdout,
           "getVnode after getNextVnode: (%x, %d, %lx)", vnodeNumber, deleted,
           pos);
                             ...
                             int *deleted, off_t *offset)
{
    ...
    *offset = ftello(stream);
    ...
    if (!GetInt32(stream, &temp1) || !GetInt32(stream, &temp2))
        return -1;
...
int dumpstream::copyVnodeData(DumpBuffer_t *dbuf)
{
    int tag = fgetc(stream);
    char buf[DIR_PAGESIZE];
    unsigned int nbytes;

    LogMsg(10, VolDebugLevel, stdout, "Copy:%s type %x",
           (IndexType == vLarge ? "Large" : "Small"), tag);
    if (IndexType == vLarge) {
        ...
        for (unsigned int i = 0; i < num_pages;
             i++) { /* copy DIR_PAGESIZE bytes to output. */
            tag = fgetc(stream);
            if (tag != 'P') {
                LogMsg(0, VolDebugLevel, stderr,
                       "Restore: Dir page does not have a P tag");
            ...
            if (fread(buf, DIR_PAGESIZE, 1, stream) != 1) {
                LogMsg(0, VolDebugLevel, stderr, "Error reading dump file %s.",
                       name);
                return -1;
            }
...
int dumpstream::CopyBytesToMemory(char *membuf, int nbytes)
{
    ...
    if (!rc) {
        LogMsg(0, VolDebugLevel, stderr,
               "Error reading blob size from dump file");
        return (-1);
    }
...
void PrintDumpHeader(FILE *outfile, struct DumpHeader *dh)
{
    ...
    fprintf(outfile, "Parent id = %08x  Timestamp = %s", dh->parentId,
            ctime(&timestamp));
