    ...
    if (newvolname[0]) { /* It's '\000' if user didn't specify a name */
        ...
        if (vldp != NULL) {
            ...
            VLog(0, "key = %s \t voltype = %d \n \tnServers = %d\tvolid = %x",
                 vldp->key, vldp->volumeType, vldp->nServers,
                 vldp->volumeId[0]);
            VDisconnectFS();
...
exit1:
    ...
    VLog(9, "S_VolClone: Created Volume %x; going to clone from %x", newId,
         originalId);

    VUCloneVolume(&error, originalvp, newvp);
    if (error) {
        VLog(0, "S_VolClone: Error while cloning volume %x -> %x", originalId,
             newId);
        V_VolLock(originalvp).IPAddress = 0;
        ReleaseWriteLock(&(V_VolLock(originalvp).VolumeLock));
        rvmlib_begin_transaction(restore);
    ...
    if (status == 0) {
        ...
        *cloneId = newId; /* set value of out parameter */
        ...
        VLog(0, "S_VolClone: volume clone failed for volume %x", originalId);
    }
...
static void VUCloneIndex(Error *error, Volume *rwVp, Volume *cloneVp,
                         VnodeClass vclass)
{
    rvm_return_t status;
    unsigned int i;
    ...
    rvlist =
        (rec_smolist *)(rvmlib_rec_malloc(sizeof(rec_smolist) * vnlistSize));
    rec_smolist *tmpvlist =
        (rec_smolist *)malloc((int)(sizeof(rec_smolist) * vnlistSize));
    ...
    if (vclass == vSmall) {
        ...
        RVMLIB_MODIFY(SRV_RVM(VolumeList[cvolInd]).data.nsmallLists,
                      vnlistSize);
    } else { /* vclass == vLarge */
        RVMLIB_MODIFY(SRV_RVM(VolumeList[cvolInd]).data.largeVnodeLists,
                      rvlist);
        RVMLIB_MODIFY(SRV_RVM(VolumeList[cvolInd]).data.nlargevnodes, nvnodes);
        RVMLIB_MODIFY(SRV_RVM(VolumeList[cvolInd]).data.nlargeLists,
                      vnlistSize);
    }
    ...
    *error         = 0;
    ...
    error:
        ...
        if (moreVnodes) {
            VLog(9, "Finished %d %s vnode clones.", MaxVnodesPerTransaction,
                 (vclass == vLarge ? "Large" : "Small"));
        } else {
            VLog(9, "Finished cloning %s vnodes.",
                 vclass == vLarge ? "Large" : "Small");
        }
               ...
               rec_smolist *rvlist, VnodeDiskObject *vnode, VnodeClass vclass)
{
    ...
    if (vclass == vLarge) { /* Directory -- no way it can be BARREN */
        ...
        if (docreate) {
            vnode->node.inodeNumber = icreate(V_device(cloneVp), V_id(rwVp),
                                              vnodeNum, vnode->uniquifier, 0);
            vnode->length =
                0; /* Reset length since we have a new null inode. */
        } else
