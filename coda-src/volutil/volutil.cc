...
extern long int volUtil_ExecuteRequest(RPC2_Handle, RPC2_PacketBuffer *,
                                       SE_Descriptor *);
...
void VolUtilLWP(void *arg)
{
    ...
    *pt = volumeUtility;
    ...
    while (1) {
        ...
        rc = RPC2_GetRequest(&myfilter, &mycid, &myrequest, NULL, VolGetKey,
                             RPC2_XOR, NULL);
        if (rc != RPC2_SUCCESS) {
        ...
        if (rc) {
            LogMsg(0, SrvDebugLevel, stdout,
                   "volutil lwp %d: request %d failed with %s", lwpid,
                   myrequest->Header.Opcode, ViceErrorMsg(rc));
        }
...
static int IsAdminUser(RPC2_CountedBS *cid)
{
    ...
    if (strncmp((char *)st->MagicString, AUTH_MAGICVALUE, sizeof(AuthMagic)) !=
        0)
        return 0;
...
{
    LogMsg(0, VolDebugLevel, stdout, "Setting Volume debug level to %d",
           debuglevel);
...
{
    LogMsg(0, VolDebugLevel, stdout, "%s ignored, use gdb", f);
    return 0;
}
