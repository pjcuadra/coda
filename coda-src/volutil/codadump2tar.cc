...
int main(int argc, char **argv)
{
    ...
    if (DEBUG_HEAVY)
        PrintDumpHeader(stderr, &ThisHead);
    ...
    if (DEBUG_HEAVY)
        PrintVolumeDiskData(stderr, &ThisVDD);
    if (DEBUG_HEAVY)
        fprintf(stderr, "\n\n");
    ...
    if (DEBUG_LIGHT)
        fprintf(stderr, "Reading and processing directories: ");
  ...
  */
    ...
    if (DEBUG_HEAVY) {
        ...
        for (i = 0; i < LVNfillcount; i++) {
            ...
            fprintf(stderr, "%s%s\n", path,
                    LVNlist[i]->children ? "" : " [empty]");
        }
    }

    /* use LVNlist to add directories to the tarfile */
    /* Incremental dumps include all directories, since tar does the same. */
    CreateDirectories();
    ...
    for (i = 0; i < vSmallCount; i++) {
        ...
        if (DEBUG_HEAVY)
            fprintf(stderr, "Ending small vnode #%i:\n", i);
        if (rc == -1)
            break;
        if (DEBUG_LIGHT)
            fprintf(stderr, "%d ", i);
...
void ParseArgs(int argc, char **argv)
{
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(argv[i], "-hp")) {
            ...
            if (HashPower > 30)
                goto BogusArgs; /* 2^30 buckets is max! */
            continue;
        }
...
BogusArgs:
    printf(
        "Usage: codadump2tar [-v] [-f <dumpfilename>] [-o <tarfilename>] [-d <debuglevel>] [-hp <hashpower>] [-rn <rootname>]\n");
    exit(EXIT_FAILURE);
}
...
int ProcessFileOrSymlink()
{
  ...
  */
    ...
    TarObj.tr_size =
        (unsigned int)smallv.length; /* set to 0 below if symlink */
    ...
    if (smallv.type == vSymlink) {
       ...
       to get sym link value before creating and writing out tar recd */
        ...
        memcpy(TarObj.tr_linkname, buff,
               (smallv.length < 100) ? smallv.length : 99);

        /* fill and output this tar record */
        TarObj.tr_type = SYMTYPE;
        TarObj.tr_size =
            0; /* override smallv.length; symlinks in tar files have zero length  */
        TarObj.Format();
        TarObj.Output();
        free(buff);
        return 0;
...
void DumpObject::AddParent(DumpObject *parent, const char *component)
{
    ...
    if (!links) {
        ...
        newparents =
            (DumpObject **)realloc(parents, newlen * sizeof(DumpObject *));
...
{
    if (dobj->oid.vnode != testid->vnode)
        return (0);
    if (dobj->oid.uniquifier != testid->uniquifier)
        return (0);
    return (1);
}
