                  ...
                  RPC2_Integer Debug, RPC2_Integer list)
{
    ...
    VLog(9, "Entering S_VolSalvage (%d, %s, %x, %d, %d, %d)", rpcid, path,
         singleVolumeNumber, force, Debug, list);
...
static int SalvageFileSys(char *path, VolumeId singleVolumeNumber)
{
    ...
    if (singleVolumeNumber) { /* not running in full salvage mode */
        ...
        VLog(0, "Salvaging file system partition %s", path);
        if (ForceSalvage)
            VLog(0, "Force salvage of all volumes on this partition");
    }
    ...
    if (rc != 0) {
        if (rc == VNOVOL) {
            ...
            VLog(9, "SalvageFileSys: GetInodeSummary failed with %d", rc);
            return rc;
        }
    } else {
    ...
    if (inodeFd == -1) {
        VLog(0, "Temporary file %s is missing...", inodeListPath);
        return (VNOVNODE);
    }

    /* get volume summaries */
    if ((rc = GetVolumeSummary(singleVolumeNumber)) != 0) {
        VLog(0, "SalvageFileSys: GetVolumeSummary failed with %d", rc);
        return (rc);
    }
    if (nVolumes > nVolumesInInodeFile)
        VLog(0, "SFS: There are some volumes without any inodes in them");
    ...
    for (i = 0, vsp = volumeSummary; i < nVolumesInInodeFile; i++) {
        ...
        while (nVolumes && (vsp->header.parent < rwvid)) {
            VLog(0,
                 "SFS:No Inode summary for volume 0x%x;"
                 " skipping full salvage",
                 vsp->header.parent);
        ...
        if (nVolumes && vsp->header.parent == rwvid) {
            ...
            int SalVolCnt           = 0;
            while (nVolumes && vsp->header.parent == rwvid) {
                VLog(9, "SFS: Setting Volume 0x%x inodesummary to %d", rwvid,
                     i);
            ...
            VLog(0, "No Volume corresponding for inodes with vid 0x%lx", rwvid);
...
static int SalvageVolumeGroup(struct VolumeSummary *vsp, int nVols)
{
    ...
    if (skipvolnums != NULL) {
        for (int i = 0; i < nVols; i++) {
            if (InSkipVolumeList(vsp[i].header.parent, skipvolnums,
                                 nskipvols) ||
                InSkipVolumeList(vsp[i].header.id, skipvolnums, nskipvols)) {
                VLog(
                    0,
                    "Volume %x listed in /vice/vol/skipsalvage and is not to be salvaged",
                    vsp[i].header.parent);
                return 0;
            }
        }
    ...
    CODA_ASSERT(
        lseek(inodeFd, isp->index * sizeof(struct ViceInodeInfo), L_SET) != -1);
    ...
    for (int i = 0; i < nVols; i++) {
        ...
        if (VnodeInodeCheck(!(readOnly(&vsp[i])), inodes,
            ...
            VLog(
                0,
                "SVG: Vnode/Inode correspondence not OK(0x%08x).... Aborting set",
                vsp[i].header.parent);
            if (inodes)
                free(inodes);
            return -1;
    ...
    VLog(9, "Leaving SalvageVolumeGroup(0x%#08x, %d)", vsp->header.parent,
         nVols);
    return 0;
}
                           ...
                           struct VolumeSummary *vsp)
{
    ...
    int vnodeIndex         = 0;
    int nVnodes            = 0;
    vindex v_index(vsp->header.id, vSmall, fileSysDevice,
    ...
    for (vnodeIndex = 0; nVnodes && ((vnodeIndex = vnext(vnode)) != -1);
         nVnodes--) {
        ...
        /* take care of special cases first */
        {
            ...
            if (vnode->node.dirNode == NEWVNODEINODE) {
                vnode->type = vNull;
                CODA_ASSERT(
                    v_index.put(vnodeNumber, vnode->uniquifier, vnode) == 0);
                VolumeChanged = 1;
                continue;
                ...
                if (vnode->cloned)
                    vnode->cloned =
                        0; // invoke COW - XXX added 9/30/92 Puneet Kumar
                CODA_ASSERT(
                    v_index.put(vnodeNumber, vnode->uniquifier, vnode) == 0);
                VolumeChanged = 1;
                continue;
        ...
        if (foundinode) { /* lip points to matching inode summary*/
            if (RW) { /* check uniquifier and data versions for RW volumes */
                ...
                if ((vu != iu) || (vd != id)) {
                    VLog(
                        ...
                        vsp->header.id, vnodeNumber, vnode->uniquifier, vu, vd,
                        iu, id);
            ...
            if (readOnly(vsp)) {
                ...
                if (!IsBarren(vnode->versionvector)) {
                    ...
                    if (!nodebarrenize) {
                        VLog(0, "Vnode 0x%x.%x.%x is BARREN - Debarrenizing\n",
                             vsp->header.id, vnodeNumber, vnode->uniquifier);
                        vnode->node.inodeNumber = icreate(
                            fileSysDevice, vsp->header.parent, vnodeNumber,
                            vnode->uniquifier, vnode->dataVersion);
...
static void CleanInodes(struct InodeSummary *isp)
{
    ...
    CODA_ASSERT(
        lseek(inodeFd, isp->index * sizeof(struct ViceInodeInfo), L_SET) != -1);
    CODA_ASSERT(read(inodeFd, (char *)inodes, size) == size);
    VLog(0, "Inodes found from destroyed volumes: scavenging.");
    for (int i = 0; i < isp->nInodes; i++) {
        ViceInodeInfo *ip = &inodes[i];
        CODA_ASSERT(ip->LinkCount > 0);
        VLog(1, "Scavenging inode %u, size %u, p=(%lx,%lx,%lx,%lx)",
             ip->InodeNumber, ip->ByteCount, ip->VolumeNo, ip->VnodeNumber,
             ip->VnodeUniquifier, ip->InodeDataVersion);
        while (ip->LinkCount > 0) {
            CODA_ASSERT(idec(fileSysDevice, ip->InodeNumber, ip->VolumeNo) ==
                        0);
            ip->LinkCount--;
        }
    }
...
static int JudgeEntry(struct DirEntry *de, void *data)
{
    ...
    VLog(39, "Entering JudgeEntry(%s (%#x.%x.%x))", name, dir->Vid, vnodeNumber,
         unique);

    vnodeEssence = CheckVnodeNumber(vnodeNumber, unique);
    if (vnodeEssence == NULL || vnodeEssence->unique != unique) {
        VLog(0, "JE: directory vnode %#x.%x.%x: invalid entry %s; ", dir->Vid,
             dir->vnodeNumber, dir->unique, name);
        VLog(
            0,
    ...
    if (strcmp(name, ".") == 0) {
        if (dir->vnodeNumber != vnodeNumber || dir->unique != unique) {
            SLog(0,
                 "JE:directory vnode %#x.%x.%x: bad '.' entry (was 0x%x.%x); ",
        ...
        if (pa.Vnode != vnodeNumber || pa.Unique != unique) {
            SLog(
                0,
                "JE: directory vnode 0x%#08x.%x.%x: bad '..' entry (was 0x%x.%x); Shouldnt Happen ",
                dir->Vid, dir->vnodeNumber, dir->unique, vnodeNumber, unique);
            CODA_ASSERT(0);
        }
        dir->haveDotDot = 1;
        ...
        if ((vnodeEssence->vparent != dir->vnodeNumber) ||
            ...
            SLog(
                0,
                "JE: parent = %#x.%x.%x ; child thinks parent is 0x%x.%x; Shouldnt Happen",
                dir->Vid, dir->vnodeNumber, dir->unique, vnodeEssence->vparent,
                vnodeEssence->uparent);
            CODA_ASSERT(0);
        }
    ...
    vnodeEssence->count--;
    VLog(39, "Leaving JudgeEntry(%s (0x%#08x.%x.%x))", name, dir->Vid,
         vnodeNumber, unique);
...
static void DistilVnodeEssence(VnodeClass vclass, VolumeId volid)
{
    ...
    if (vip->nVnodes > 0) {
        vip->vnodes = (struct VnodeEssence *)calloc(
            vip->nVnodes, sizeof(struct VnodeEssence));
        CODA_ASSERT(vip->vnodes != NULL);
        if (vclass == vLarge) { /* alloc space for directory entries */
            vip->dirnodes =
                (PDirInode *)calloc(vip->nVnodes, sizeof(PDirInode));
        ...
        if (vnode->type != vNull) {
            ...
            if (vnode->type == vDirectory) {
                ...
                if (vnode->node.dirNode == NEWVNODEINODE) {
                    ...
                    VLog(
                        0,
                        "DistilVnodeEssence: Found a Directory"
...
void DirCompletenessCheck(struct VolumeSummary *vsp)
{
    ...
    for (vclass = 0; vclass < nVNODECLASSES; vclass++) {
        ...
        for (i = 0; i < nVnodes; i++) {
            ...
            VLog(29,
                 "\t linkcount = %d, index = %d, parent = 0x%x, unique = 0x%x",
                 vnp->count, i, vnp->vparent, vnp->unique);
            if (vnp->changed || vnp->count != 0) {
                VLog(
                    0,
                    "DCC: For Vnode (%#x %#x %#x) parent (%#x %#x %#x): linkcount is %d bigger than count of directory entries.",
                    vid, vnp->vid, vnp->unique, vid, vnp->vparent, vnp->uparent,
                    vnp->count);
                doassert = 1;
...
static int AskOffline(VolumeId volumeId)
{
    ...
    *pt = tmp;
    if (rc == FSYNC_DENIED) {
        VLog(
            0,
            "AskOffline:  file server denied offline request; a general salvage is required.");
...
static int AskOnline(VolumeId volumeId)
{
    ...
    *pt = tmp;
    if (rc == FSYNC_DENIED) {
        VLog(
            0,
            "AskOnline:  file server denied online request; a general salvage is required.");
    }
    return (0);
}
...
static void PrintInodeList()
{
    ...
    for (ip = buf; nInodes--; ip++) {
        VLog(0, "Inode:%u, linkCount=%d, size=%u, p=(%lx,%lx,%lx,%lx)",
             ip->InodeNumber, ip->LinkCount, ip->ByteCount, ip->VolumeNo,
             ip->VnodeNumber, ip->VnodeUniquifier, ip->InodeDataVersion);
    }
    free((char *)buf);
}
#endif
...
static void SanityCheckFreeLists()
{
    ...
    for (i = 0; i < SRV_RVM(SmallVnodeIndex); i++) {
        if (memcmp((const void *)SRV_RVM(SmallVnodeFreeList[i]),
                   (const void *)zerovn, SIZEOF_SMALLDISKVNODE) != 0) {
    ...
    for (i = 0; i < SRV_RVM(LargeVnodeIndex); i++) {
        if (memcmp(SRV_RVM(LargeVnodeFreeList[i]), zerovn,
            ...
            if (i != SRV_RVM(LargeVnodeIndex))
                RVMLIB_MODIFY(
                    SRV_RVM(LargeVnodeFreeList[i]),
                    SRV_RVM(LargeVnodeFreeList[SRV_RVM(LargeVnodeIndex)]));

            /* And now we can clear the tail of the freelist */
            RVMLIB_MODIFY(SRV_RVM(LargeVnodeFreeList[SRV_RVM(LargeVnodeIndex)]),
                          NULL);
    ...
    for (ip = inodes; totalInodes; ip++, totalInodes--) {
        ...
        if (ip->LinkCount != 0 && TraceBadLinkCounts) {
            ...
            VLog(
                ...
                ip->LinkCount, ip->InodeNumber, ip->ByteCount, ip->VolumeNo,
                ip->VnodeNumber, ip->VnodeUniquifier, ip->InodeDataVersion);
        }
        ...
        while (ip->LinkCount < 0) {
            CODA_ASSERT(iinc(fileSysDevice, ip->InodeNumber, ip->VolumeNo) ==
                        0);
            ip->LinkCount++;
        }
    }
...
static void CountVolumeInodes(struct ViceInodeInfo *ip, int maxInodes,
                           ...
                           VolumeId singleVolumeNumber)
{
    ...
    if (dp == NULL) {
        VLog(0, "Cannot find partition %s\n", path);
        return VFAIL;
    }

    VLog(9, "Entering GetInodeSummary (%s, %x)", path, singleVolumeNumber);

    if (singleVolumeNumber)
        rc = dp->ops->ListCodaInodes(dp, path, OnlyOneVolume,
                                     singleVolumeNumber);
    else
        rc = dp->ops->ListCodaInodes(dp, path, NULL, singleVolumeNumber);

    if (rc == 0) {
        VLog(9, "ListViceInodes returns success");
    }
    ...
    CODA_ASSERT(read(summaryFd, (char *)inodeSummary, status.st_size) ==
                status.st_size);
...
static int CompareVolumes(struct VolumeSummary *p1, struct VolumeSummary *p2)
{
...
static int GetVolumeSummary(VolumeId singleVolumeNumber)
{
    ...
    VLog(9, "Entering GetVolumeSummary(%x)", singleVolumeNumber);

    /* Make sure the disk partition is readable */
    if (access(fileSysPath, R_OK) != 0) {
        VLog(0, "Can't read directory %s; not salvaged", fileSysPath);
        return (VNOVNODE);
    }

    /* iterate through all the volumes on this partition, and try to */
    /* match with the desired volumeid */
    VLog(39, "GetVolSummary: filesyspath = %s nVolumes = %d", fileSysPath,
         nVolumes);
    for (i = 0; i <= MAXVOLS; i++) {
        ...
        /* Sanity checks */
        CODA_ASSERT(vsp->header.id ==
                    SRV_RVM(VolumeList[i]).data.volumeInfo->id);
        CODA_ASSERT(vsp->header.parent ==
                    SRV_RVM(VolumeList[i]).data.volumeInfo->parentId);

        for (int j = i + 1; j < MAXVOLS; j++)
            if (vsp->header.id == SRV_RVM(VolumeList[j]).header.id)
                CODA_ASSERT(0);

        /* reject volumes from other partitions */
        VLog(39, "Partition for vol-index %d, (id 0x%x) is (%s)", i,
             vsp->header.id,
        ...
        VLog(9,
             "GetVolSummary: inserting volume %x into hashtable for index %d",
             vsp->header.id, i);
        ...
        if (singleVolumeNumber && vsp->header.id == singleVolumeNumber) {
            if (vsp->header.type == readonlyVolume) { // skip readonly volumes
                VLog(0, "%d is a read-only volume; not salvaged",
                     singleVolumeNumber);
                return (VREADONLY);
            }
