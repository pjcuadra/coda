                  ...
                  RPC2_String formal_volname, RPC2_Unsigned *formal_volid)
{
    ...
    VDisconnectFS();
    VLog(2, "Restore took %d seconds to dump %d bytes.", dbuf->secs,
         dbuf->nbytes);
    free(dbuf);
    free(DumpBuf);

    if (RPC2_Unbind(cid) != RPC2_SUCCESS) {
        VLog(0, "S_VolNewDump: Can't close binding %s", RPC2_ErrorMsg((int)rc));
    }

    return (status ? status : rc);
}

static int RestoreVolume(DumpBuffer_t *buf, char *partition, char *volname,
                         VolumeId *volid)
{
    VolumeDiskData vol;
    ...
    VLog(9, "Volume id = %x, Volume name = %s", header.volumeId,
         header.volumeName);
    backupdate = (time_t)header.backupDate;
    VLog(9, "Dump from backup taken from %x at %s", header.parentId,
         ctime(&backupdate));
    VLog(9, "Dump uniquifiers %x -> %x", header.oldest, header.latest);
...
error:
    ...
    if (strlen(volname) == 0) {
        AssignVolumeName(&vol, vol.name,
                         (volumeType == readonlyVolume ? ".readonly" :
                                                         ".restored"));
    } else {
...
static int ReadLargeVnodeIndex(DumpBuffer_t *buf, Volume *vp)
{
    ...
    rec_smolist *vmrlist =
        (rec_smolist *)malloc(sizeof(rec_smolist) * list_size);
    ...
    while (i < num_vnodes) {
        ...
        do {
            ...
            if (vdo->type != vNull) {
                ...
                camvdo =
                    (VnodeDiskObject *)rvmlib_rec_malloc(SIZEOF_LARGEDISKVNODE);
                rvmlib_modify_bytes(&(camvdo->nextvn), &tmp, sizeof(long));
                rlist[vnodeIdToBitNumber(vnodenumber)].append(
                    &(camvdo->nextvn));
                vdo->cloned     = 0;
    ...
    while (i < num_vnodes) {
        ...
        do {
            ...
            if (vdo->type != vNull) {
                /* this vnode is allocated */
                camvdo =
                    (VnodeDiskObject *)rvmlib_rec_malloc(SIZEOF_SMALLDISKVNODE);
                rvmlib_modify_bytes(&(camvdo->nextvn), &tmp, sizeof(long));
                rlist[vnodeIdToBitNumber(vnodenumber)].append(
                    &(camvdo->nextvn));
                vdo->cloned     = 0;
    ...
    if (vdop->type == vDirectory) {
        ...
        *dinode = (PDirInode)malloc(sizeof(struct DirInode));
        ...
        for (int i = 0; i < npages; i++) {
            ...
            if (!ReadByteString(buf, (char *)((*dinode)->di_pages[i]),
                                DIR_PAGESIZE)) {
