...
static int DumpDumpHeader(DumpBuffer_t *dbuf, Volume *vp,
                          RPC2_Unsigned Incremental, int oldunique, int unique)
{
...
static int DumpVolumeDiskData(DumpBuffer_t *dbuf, VolumeDiskData *vol)
{
    ...
    DumpArrayInt32(dbuf, 'W', (unsigned int *)vol->weekUse,
                               ...
                               int vnodeNumber, Device device)
{
    ...
    if (v->type != vDirectory) {
        ...
        for (i = 0; i < size; i++) {
            if (DumpByteString(dbuf, 'P', (char *)DI_Page(dip, i),
                               DIR_PAGESIZE) == -1)
                return -1;
        }
                          ...
                          FILE *Ancient)
{
    ...
    if (write(VVListFd, buf, strlen(buf)) != (int)strlen(buf)) {
        SLog(0, "Write %s Index header didn't succeed.", vclass_str);
        VPutVolume(vp);
        return -1;
    }
    ...
    if (Incremental) {
        ...
        do {
            ...
            if (fgets(buf, LISTLINESIZE, Ancient) == NULL) {
                SLog(0, "Dump: Failed to find %s list header.", vclass_str);
                VPutVolume(vp);
                return -1;
            }

            /* Read in enough info from vvlist file to start a vvlist class
	     * object. */
            if (sscanf(buf, "Start of %s list, %ld vnodes, %d lists.\n", Class,
                       &nvnodes, &nlists) != 3)
                continue;
        } while (strcasecmp(Class, vclass_str) != 0);
        ...
        if (vclass == vLarge)
            vnList =
                SRV_RVM(VolumeList[V_volumeindex(vp)]).data.largeVnodeLists;
        else
            vnList =
                SRV_RVM(VolumeList[V_volumeindex(vp)]).data.smallVnodeLists;
	 ...
	 */
        ...
        for (int vnodeIndex = 0; vnodeIndex < (int)nLists; vnodeIndex++) {
            ...
            while ((vptr = nextVnode())) { /* While more vnodes */
                ...
                if (vvlist.IsModified(vnodeIndex, vnode->uniquifier,
                    ...
                    if (DumpVnodeDiskObject(dbuf, vnode, VnodeNumber,
                                            V_device(vp)) == -1) {
                        SLog(0, 0, stdout, "DumpVnodeDiskObject (%s) failed.",
                             vclass_str);
                        return -1;
                    }
                    SLog(9, "Dump: Incremental found %x.%x modified.",
	     ...
	     */
		     ...
		     */
                    ...
                    if (DumpDouble(dbuf, D_RMVNODE, VnodeNumber,
                                   vventry->unique) == -1) {
                        SLog(0, 0, stdout, "Dump RMVNODE failed, aborting.");
                        return -1;
                    }
                    SLog(9, "Dump: Incremental found %x.%x deleted.",
        ...
        for (int vnodeIndex = 0; nVnodes && ((vnodeIndex = vnext(vnode)) != -1);
             nVnodes--, count++) {
            int VnodeNumber = bitNumberToVnodeNumber(vnodeIndex, vclass);
            if (DumpVnodeDiskObject(dbuf, vnode, VnodeNumber, V_device(vp)) ==
                -1) {
                SLog(0, "DumpVnodeDiskObject (%s) failed.", vclass_str);
                return -1;
            }
                                   ...
                                   RPC2_Unsigned *sizes, FILE *Ancient)
{
    ...
    if (Ancient) {
        ...
        CODA_ASSERT(strcmp(Class, ((vclass == vLarge) ? "Large" : "Small")) ==
                    0);
        ...
        while ((vptr = nextVnode())) { /* While more vnodes */
            ...
            if (Ancient)
                vvl->IsModified(vnodeIndex, vnode->uniquifier,
                                &(vnode->versionvector.StoreId), 9, &dumplevel);
                  ...
                  RPC2_Integer *Incremental)
{
    ...
    RPC2_Unsigned SmallIncr, LargeIncr;
    int VVListFd  = -1;
    FILE *Ancient = NULL;
    ...
    SLog(9, "S_VolNewDump: conn: %d, volume:  %#x, Inc?: %d", rpcid,
         volumeNumber, *Incremental);
    ...
    SLog(0, "NewDump: file %s volnum %x id %x parent %x", VVlistfile, volnum,
         volumeNumber, V_parentId(vp));
    ...
    if (*Incremental) {
        ...
        if (Ancient && !ValidListVVHeader(Ancient, vp, &oldunique)) {
            fclose(Ancient);
            Ancient = NULL;
        }
        ...
        if (Ancient == NULL) {
            SLog(0,
                 "S_VolDump: Couldn't open Ancient vvlist %s, "
                 "will do a full backup instead.",
                 listfile);
            *Incremental = 0;
        }
    }
    ...
    if (*Incremental < 0) {
        LargeIncr = 0;
        SmallIncr = -(*Incremental);
    } else {
        LargeIncr = *Incremental;
        SmallIncr = *Incremental;
    }
    ...
    /* Dump the volume.*/
    DumpListVVHeader(VVListFd, vp, SmallIncr, unique);
    if ((DumpDumpHeader(dbuf, vp, SmallIncr, oldunique, unique) == -1) ||
        (DumpVolumeDiskData(dbuf, &V_disk(vp)) == -1) ||
        (DumpVnodeIndex(dbuf, vp, vLarge, LargeIncr, VVListFd, Ancient) ==
         -1) ||
        (DumpVnodeIndex(dbuf, vp, vSmall, SmallIncr, VVListFd, Ancient) ==
         -1) ||
        (DumpEnd(dbuf) == -1)) {
...
failure:
    ...
    if (dbuf) {
        SLog(2, "Dump took %d seconds to dump %d bytes.", dbuf->secs,
             dbuf->nbytes);
        free(dbuf);
    }
    if (DumpBuf)
    ...
    unlink(VVlistfile);
    SLog(0, "S_VolNewDump: %svolume dump failed with status = %d",
