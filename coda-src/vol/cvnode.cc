...
static void GrowVnLRUCache(VnodeClass vclass, int nVnodes)
{
    ...
    SLog(9, "Entering GrowVnLRUCache(vclass = %d, vnodes = %d)", vclass,
         nVnodes);
    va = (byte *)calloc(nVnodes, vcp->residentSize);
    CODA_ASSERT(va != NULL);
    vcp->cacheSize += nVnodes;
                                ...
                                VnodeId vnode, Unique_t unique)
{
    ...
    if ((vp->vnIndex[vclass].bitmapSize << 3) > vol_index.elts()) {
        rvm_return_t camstatus;
        VLog(1, "VAllocVnode: growing %s vnode array",
             vclass ? "small" : "large");
        rvmlib_begin_transaction(restore);
    ...
    if (ObjectExists(V_volumeindex(vp), vclass, vnodeIdToBitNumber(vnode),
                     unique)) {
        VLog(0, "VAllocVnode: object (%08x.%x.%x) found in RVM", V_id(vp),
             vnode, unique);
        *ec = EEXIST;
        return (NULL);
    }
    ...
    for (vnp = VnodeHashTable[newHash];
         vnp && (vnp->vnodeNumber != vnode || vnp->volumePtr != vp ||
    ...
    if (vnp != NULL) {
        LogMsg(0, VolDebugLevel, stdout,
               "VAllocVnode: object (%08x.%x.%x) found in VM", V_id(vp), vnode,
               unique);
        *ec = EEXIST;
        return (NULL);
    }
    ...
    if (vcp->lruHead->lruPrev == vcp->lruHead) {
        LogMsg(
            ...
            "VAllocVnode: LRU cache has only one entry - growing cache dynamically");
    ...
    LogMsg(19, VolDebugLevel, stdout,
           "VAllocVnode: printing vnode %x after allocation:", vnode);
    if (VolDebugLevel >= 19)
...

{
    ...
    *ec = 0;
    ...
    if (vnp == NULL) {
        ...
        SLog(1, "VGetVnode: going to rvm for vnode %08x.%x", V_id(vp),
             vnodeNumber);
        vcp->reads++;
        ...
        if (vnp->dh) {
            SLog(0,
                 "VGetVnode: DROPPING dh of vn %x un %x"
                 "total count %d, dh_refc: %d\n",
                 vnp->vnodeNumber, vnp->disk.uniquifier, DC_Count(vnp->dh),
                 vnp->dh_refc);
        ...
        if ((n = v_index.get(vnodeNumber, unq, &vnp->disk)) != 0) {
            ...
            *ec = VNOVNODE;
            SLog(0, "VGetVnode: vnode %08x.%x is not allocated", V_id(vp),
                 vnodeNumber);
            return NULL;
        }
        ...
        if (vnp->disk.type == vNull) {
            *ec = VNOVNODE;
            SLog(0, "VGetVnode: vnode %08x.%x not allocated", V_id(vp),
                 vnodeNumber);
            return NULL;
        }
        if (vnp->disk.vnodeMagic != vcp->magic) {
            SLog(
                0,
                "VGetVnode: Bad vnodeMagic, vnode %08x.%x, (%s); volume needs salvage",
    ...
    *ec = 0;
    ...
    if (writeLocked) {
        ...
        if (thisProcess != vnp->writer) {
            SLog(-1, "VPutVnode: Vnode %x locked by another process!",
                 vnp->vnodeNumber);
            CODA_ASSERT(0);
        }
        ...
        if (vnp->changed || vnp->delete_me) {
            ...
            if (!V_inUse(vp)) {
                ...
                *ec = VSALVAGE;
                ...
                if (vnp->delete_me && v_index.IsEmpty(vnp->vnodeNumber))
        ...
        if (vnp->changed || vnp->delete_me) {
            SLog(
                -1,
                "VPutVnode: Change or delete flag for vnode %x is set but vnode is not write locked!",
                vnp->vnodeNumber);
            CODA_ASSERT(0);
        }
    }
...
void VFlushVnode(Error *ec, Vnode *vnp)
{
    ...
    *ec = 0;
    ...
    if (ObjectExists(V_volumeindex(vp), vclass,
                     vnodeIdToBitNumber(vnp->vnodeNumber),
                     vnp->disk.uniquifier)) {
        CODA_ASSERT(v_index.get(vnp->vnodeNumber, vnp->disk.uniquifier,
                                &vnp->disk) == 0);
    } else {
...
static void moveHash(Vnode *vnp, bit32 newHash)
{
    ...
    LogMsg(9, VolDebugLevel, stdout, "Entering moveHash(vnode %x)",
           vnp->vnodeNumber);
    tvnp = VnodeHashTable[vnp->hashIndex];
    if (tvnp == vnp) {
        SLog(9, "moveHash: setting VnodeHashTable[%d] = %p", vnp->hashIndex,
             vnp->hashNext);
