...
void checkvm()
{
    ...
    for (i = 0, j = 4096; j < rds_rvmsize; i += 4096, j += 4096, p += 4096) {
        if (read(fd, buf, 4096) != 4096) {
            LogMsg(0, VolDebugLevel, stdout, "Read failed i %d, err %d", i,
                   errno);
            readerror = 1;
            break;
        }
            ...
            for (int k = 0; k < 4096; k += 4)
                if (memcmp(&buf[k], &p[k], 4)) {
                    LogMsg(
                        ...
                        "CheckVM: Addr = 0x%x Dataseg value = 0x%x, Dumpfile value = 0x%x",
                        &p[k], *((int *)&p[k]), (*(int *)&buf[k]));
                    errorsfound++;
                }
    }

    if (readerror && !errorsfound)
        return;
    ...
    if (!readerror && nbytes) {
        if (read(fd, buf, nbytes) != nbytes) {
            LogMsg(0, VolDebugLevel, stdout,
                   "Checkvm: Read failed address 0x%x size = %d", p, nbytes);
            readerror = 1;
        }
    ...
    if (errorsfound) {
        LogMsg(0, VolDebugLevel, stdout, "CheckVM: Number of errors = %d",
               errorsfound);
        CODA_ASSERT(0);
    }
    close(fd);
...
int coda_init()
{
    ...
    rvmlib_set_range(SRV_RVM(SmallVnodeFreeList),
                     SMALLFREESIZE * sizeof(VnodeDiskObject *));
    memset(SRV_RVM(SmallVnodeFreeList), 0,
           SMALLFREESIZE * sizeof(VnodeDiskObject *));
    RVMLIB_MODIFY(SRV_RVM(SmallVnodeIndex), 0);
    VLog(29, "Storing SmallVnodeIndex = %d", SRV_RVM(SmallVnodeIndex));

    rvmlib_set_range(SRV_RVM(LargeVnodeFreeList),
                     LARGEFREESIZE * sizeof(VnodeDiskObject *));
    memset(SRV_RVM(LargeVnodeFreeList), 0,
           LARGEFREESIZE * sizeof(VnodeDiskObject *));
...
void CheckVolData(Error *ec, int volindex)
{
    ...
    *ec = 0;
    ...
    LogMsg(9, VolDebugLevel, stdout, "Entering CheckVolData for volindex %d",
           volindex);

    maxid = (SRV_RVM(MaxVolId) & 0x00FFFFFF);
    if (volindex < 0 || volindex > (int)maxid || volindex > MAXVOLS) {
        LogMsg(0, VolDebugLevel, stdout, "CheckVolData: bogus volume index %d",
               volindex);
    ...
    if (data->volumeInfo->stamp.magic != VOLUMEINFOMAGIC)
        LogMsg(0, VolDebugLevel, stdout,
               "CheckVolumeData: bogus VolumeDiskData for volume %d, index %d!",
...
int ActiveVnodes(int volindex, int vclass)
{
    ...
    LogMsg(9, VolDebugLevel, stdout,
           "Entering ActiveVnodes for index %d, vclass = %d", volindex, vclass);
    maxid = (SRV_RVM(MaxVolId) & 0x00FFFFFF);
    if (volindex < 0 || volindex > (int)maxid || volindex > MAXVOLS) {
        LogMsg(0, VolDebugLevel, stdout, "ActiveVnodes: bogus volume index %d",
               volindex);
...
int AllocatedVnodes(int volindex, int vclass)
{
    ...
    LogMsg(9, VolDebugLevel, stdout,
           "Entering AllocatedVnodes for index %d, vclass %d", volindex,
           vclass);
    maxid = (SRV_RVM(MaxVolId) & 0x00FFFFFF);
    if (volindex < 0 || volindex > (int)maxid || volindex > MAXVOLS) {
        LogMsg(0, VolDebugLevel, stdout,
               "AllocatedVnodes: bogus volume index %d", volindex);
        return (-1); // invalid volume index
    }
                     ...
                     char partition[V_MAXPARTNAMELEN])
{
    ...
    *ec = 0;
    ...
    LogMsg(9, VolDebugLevel, stdout, "Entering GetVolPartition for volid %x",
           volid);
    if ((myind < 0) || (myind > MAXVOLS) ||
        (SRV_RVM(VolumeList[myind]).header.id != volid)) {
        LogMsg(0, VolDebugLevel, stdout,
               "GetVolPartition: bad index %d for volume %x", myind, volid);
        *ec = VNOVOL;
        return;
    }
...
void GrowVnodes(VolumeId volid, int vclass, unsigned short newBMsize)
{
    ...
    LogMsg(9, VolDebugLevel, stdout,
           "Entering GrowVnodes for volid %x, vclass %d", volid, vclass);

    if ((myind = HashLookup(volid)) == -1) {
        LogMsg(0, VolDebugLevel, stdout,
               "GrowVnodes: bogus volume id %x (not in hash table)", volid);
        CODA_ASSERT(0);
    }
    ...
    LogMsg(0, VolDebugLevel, stdout,
           "GrowVnodes: growing %s list from %u to %u for volume 0x%x", name,
           size, newsize, volid);
    ...
    if (vclass == vSmall) {
        ...
        rvmlib_modify_bytes(
            newvlist, SRV_RVM(VolumeList[myind]).data.smallVnodeLists,
        ...
        /* copy the existing vnode pointers into the new list */
        rvmlib_modify_bytes(
            newvlist, SRV_RVM(VolumeList[myind]).data.largeVnodeLists,
...
void ExtractVolDiskInfo(Error *ec, int volindex, VolumeDiskData *vol)
{
    ...
    *ec            = 0;
    ...
    LogMsg(9, VolDebugLevel, stdout,
           "Entering ExtractVolDiskInfo for volindex %u", volindex);

    maxid = (SRV_RVM(MaxVolId) & 0x00FFFFFF);
    if (volindex < 0 || volindex > (int)maxid || volindex > MAXVOLS) {
        LogMsg(0, VolDebugLevel, stdout,
               "ExtractVolDiskInfo: bogus volume index %d", volindex);
        *ec = VNOVOL; // invalid volume index
        return;
    }
    ...
    if (vol->stamp.magic != VOLUMEINFOMAGIC ||
        ...
        LogMsg(0, VolDebugLevel, stdout,
               "ExtractVolDiskInfo: bogus version stamp!");
        *ec = VSALVAGE;
        LogMsg(0, VolDebugLevel, stdout,
               "recoverable version stamp for volindex %x = %u, %u", volindex,
