...
int ExtractVnode(int volindex, int vclass, VnodeId vnodeindex,
                 Unique_t uniquifier, VnodeDiskObject *vnode)
{
    rec_smolist *vlist;
    VnodeDiskObject *vdo;
    VolumeId maxid;
    ...
    VLog(
        9,
        "Entering ExtractVnode(volindex = %d, vclass = %d, vnodeindex = %d, Unique = %x)",
        volindex, vclass, vnodeindex, uniquifier);
    ...
    if (!vdo) {
        VLog(9, "ExtractVnode: NO object %d.%x", vnodeindex, uniquifier);
        return -1;
    }
    ...
    if (VolDebugLevel > 19) {
        printf("Extracted %s vnode %x, (index %u) from volume %u\n",
               (vclass == vLarge) ? "large" : "small",
               bitNumberToVnodeNumber(vnodeindex, vclass), vnodeindex,
               volindex);
                 ...
                 ViceFid *ParentFid)
{
    ...
    VLog(9, "Entering ObjectExists(volindex= %d, (%x.%x)", volindex, vnodeindex,
         u);
    ...
    if (vclass == vSmall) {
        if (vnodeindex >= SRV_RVM(VolumeList[volindex]).data.nsmallLists) {
            VLog(0, "ObjectExists: bogus small vnode index %x", vnodeindex);
            return 0;
        }
        vlist =
            &(SRV_RVM(VolumeList[volindex]).data.smallVnodeLists[vnodeindex]);
        ...
        if (vnodeindex >= SRV_RVM(VolumeList[volindex]).data.nlargeLists) {
            VLog(0, "ObjectExists: bogus large vnode index %x", vnodeindex);
            return 0;
        }
        vlist =
            &(SRV_RVM(VolumeList[volindex]).data.largeVnodeLists[vnodeindex]);
                 ...
                 VnodeDiskObject *vnode)
{
    ...
    if (vnode->type == vNull) {
        VLog(9, "ReplaceVnode: removed vnode %d.%x, deleting", vnodeindex, u);
    ...
    if (vdo == NULL) {
        VLog(39, "ReplaceVnode: no %s vnode at index %d; allocating", name,
             vnodeindex);
        ...
        if (memcmp(&(vdo->nextvn), buf, sizeof(struct rec_smolink)) != 0) {
            VLog(0,
                 "ERROR: REC_SMOLINK ON VNODE DURING ALLOCATION WAS NOT ZERO");
            rvmlib_modify_bytes(&(vdo->nextvn), buf,
                                sizeof(struct rec_smolink));
        }
...
static int DeleteVnode(int volindex, int vclass, VnodeId vnodeindex, Unique_t u,
                       VnodeDiskObject *vnode)
{
    VolumeId maxid = 0;
    rec_smolist *vlist;
...
void ReplaceVolDiskInfo(Error *ec, int volindex, VolumeDiskData *vol)
{
    ...
    *ec            = 0;
    ...
    rvmlib_modify_bytes(SRV_RVM(VolumeList[volindex]).data.volumeInfo, vol,
