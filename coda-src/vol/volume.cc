...
int VInitVolUtil(ProgramType pt)
{
    ...
    if (pt != salvager) {
        ...
        if (myflock(fslock, MYFLOCK_EX, MYFLOCK_NB) != 0) {
            VLog(
                0,
                "VInitVolUtil: File Server is running: can't run full salvage");
            close(fslock);
            close(fvlock);
            return (VFAIL);
        }
...
void VInitVolumePackage(int nLargeVnodes, int nSmallVnodes, int DoSalvage)
{
    ...
    VLog(9, "Entering VInitVolumePackage(%d, %d, %d)", nLargeVnodes,
         nSmallVnodes, DoSalvage);
    ...
    /* Attach all valid volumes (from all vice partitions) */
    {
        ...
        for (i = 0; (i < maxid) && (i < MAXVOLS); i++) {
            ...
            if (HashInsert(header.id, i) != -1)
                VLog(0, "VInitVolPackage: Volume %x was not yet in the hash!",
                     header.id);
            ...
            if (error == VOFFLINE)
                VLog(0, "Volume %x stays offline (%s/%s exists)", header.id,
                     vice_config_path("offline"),
...
void VInitServerList(const char *host)
{
    ...
    while (fgets(line, sizeof(line), file) != NULL) {
        ...
        if (sscanf(line, "%50s %u", sname, &sid) == 2) {
            if (sid >= N_SERVERIDS) {
                VLog(
                    0,
                    "Host %s is assigned a bogus server number (%x) in %s. Exit.",
            ...
            if (sid == 0 || sid == 127 || sid == 255) {
                VLog(
                    0,
                    "Warning: host %s is using a reserved server number (%lu) in %s. Exit",
                    sname, sid, serverList);
                exit(EXIT_FAILURE);
            }
            ...
            if (Server_FQDN[sid]) {
                VLog(0,
                     "Fatal: unable to map server-id %d to host %s,\n"
            ...
            if (!hostent || hostent->h_length != sizeof(struct in_addr) ||
                ...
                VLog(
                    0,
                    "Host %s (listed in %s) cannot be resolved (to an IPv4 address). Exiting.",
                    sname, serverList);
                exit(EXIT_FAILURE);
            }
    ...
    if (!ThisServerId) {
        VLog(0, "Hostname of this server (%s) is not listed in %s. Exiting.",
             ThisHost, serverList);
        exit(EXIT_FAILURE);
    }
    fclose(file);
...
void VGetVolumeInfo(Error *ec, char *key, VolumeInfo *info)
{
    ...
    *ec = 0;
    ...
    if (nReported == 0) {
        *ec = VNOVOL;
        VLog(9, "VGetVolumeInfo: no reported servers for volume %x", info->Vid);
        return;
    }
...
static void VListVolume(char **buf, unsigned int *buflen, unsigned int *offset,
                        Volume *vp)
{
...
retry:
    n = snprintf(*buf + *offset, *buflen - *offset,
                 ...
                 V_minquota(vp), V_maxquota(vp), V_diskused(vp), V_parentId(vp),
                 V_creationDate(vp), V_copyDate(vp), V_backupDate(vp),
                 volumeusage);

    /* hack, snprintf sometimes fucks up and doesn't return -1 */
    if (n >= (int)(*buflen - *offset))
        n = -1;
...
void dumpvm()
{
    ...
    for (i = 0, j = 102400; j < rds_rvmsize;
         i += 102400, j += 102400, p += 102400) {
    ...
    if (nbytes)
        if (write(fd, (char *)p, (int)nbytes) != nbytes) {
            VLog(0, "Write failed for address 0x%x size %d", p, nbytes);
        }
    ...
    if (DumpVM) {
        ...
        if (RvmType == RAWIO || RvmType == UFS) {
            ...
            if (ret != RVM_SUCCESS)
                ...
                VLog(0, 0, stdout, "Uncommitted transactions: %d",
                     curopts.n_uncommit);

                for (i = 0; i < curopts.n_uncommit; i++) {
                    rvm_abort_transaction(&(curopts.tid_array[i]));
                    if (ret != RVM_SUCCESS)
                        VLog(0, 0, stdout, "ERROR: abort failed, code: %s",
                             rvm_return(ret));
                }

                ret = rvm_query(&curopts, NULL);
                if (ret != RVM_SUCCESS)
                    VLog(0, 0, stdout, "rvm_query returned %s",
                         rvm_return(ret));
                else
                    VLog(0, 0, stdout, "Uncommitted transactions: %d",
                         curopts.n_uncommit);
...
Volume *VAttachVolumeById(Error *ec, char *partition, VolumeId volid, int mode)
{
    ...
    *ec = 0;
    ...
    if (header.stamp.version != VOLUMEHEADERVERSION) {
        VLog(
            0,
            "VAttachVolumeById: Volume %s, version number %u is incorrect; volume needs salvage",
            name, header.stamp.version);
        *ec = VSALVAGE;
        return NULL;
    }
    if (*pt == volumeUtility && mode != V_SECRETLY) {
        ...
        *pt = volumeUtility;
        if (rc == FSYNC_DENIED) {
            VLog(
                0,
                "VAttachVolumeById: attach of volume %x apparently denied by file server",
                header.id);
            *ec = VNOVOL; /* XXXX */
            return NULL;
        }
    ...
    if (*pt == volumeUtility && vp == NULL && mode != V_SECRETLY) {
        ...
        *pt = volumeUtility;
        ...
        VLog(0, "VAttachVolumeById: vol %x (%s) attached and online", V_id(vp),
             V_name(vp));
    }
...
static Volume *attach2(Error *ec, char *name, struct VolumeHeader *header,
                       struct DiskPartition *dp)
{
    Volume *vp;
    ProgramType *pt;
    ...
    if (V_needsSalvaged(vp)) {
        VLog(
            0,
            "VAttachVolume: volume salvage flag is ON for %s; volume needs salvage",
            name);
        *ec = VSALVAGE;
        return NULL;
    }
    if (*pt == fileServer) {
        ...
        if (V_destroyMe(vp) == DESTROY_ME) {
            FreeVolume(vp);
            VLog(0,
                 "VAttachVolume: volume %s should be destoyed at next salvage",
                 name);
            *ec = VNOVOL;
            return NULL;
        }
...
Volume *VAttachVolume(Error *ec, VolumeId volumeId, int mode)
{
    ...
    if ((myind = HashLookup(volumeId)) == -1) {
        VLog(0, "VAttachVolume: Volume %x not in index hash table!", volumeId);
        *ec = VNOVOL;
        return NULL;
    }
...
Volume *VGetVolume(Error *ec, VolumeId volumeId)
{
    ...
    for (;;) {
        *ec = 0;
        ...
        if (vp->shuttingDown) {
            VLog(29, "VGetVolume: volume %x is shutting down", V_id(vp));
            *ec = VNOVOL;
            vp  = 0;
            break;
        }
        if (vp->goingOffline) {
            VLog(29, "VGetVolume: Volume %x is going offline", V_id(vp));
            LWP_WaitProcess((char *)VPutVolume);
            continue;
        }
        if (vp->specialStatus) {
            VLog(29, "VGetVolume: Volume %x has special status", V_id(vp));
            *ec = vp->specialStatus;
        } else if (V_inService(vp) == 0 || V_blessed(vp) == 0) {
            VLog(29, "VGetVolume: Vol %x not in service", V_id(vp));
...
void VPutVolume(Volume *vp)
{
    ...
    if (vp->nUsers == 0) {
        ...
        if (vp->goingOffline) {
            ...
            if (VolDebugLevel) {
                VLog(0, "VPutVolume: Volume %x (%s) is now offline", V_id(vp),
                     V_name(vp));
        ...
        VLog(1, "VPutVolume (%x): NO-OP since vp->nUsers = %d", V_id(vp),
             vp->nUsers + 1);
}
...
void VForceOffline(Volume *vp)
{
    ...
    V_inUse(vp) = 0;
    VLog(1,
         "VForceOffline: setting V_inUse(%x) = 0 and writing out voldiskinfo",
         V_id(vp));
    vp->goingOffline    = 0;
...
void VOffline(Volume *vp, const char *message)
{
    ...
    CODA_ASSERT(LWP_GetRock(FSTAG, &rock) == LWP_SUCCESS);
    pt = (ProgramType *)rock;
    VLog(9, "Entering VOffline for volume %x, running as %s", V_id(vp),
         (*pt == fileServer) ?
             "fileServer" :
             ((*pt == volumeUtility) ? "volumeUtility" : "fileUtility"));
                      ...
                      int stride, int ix, int count)
{
    *ec = 0;
    ...
    byte *bp = index->bitmap +
               index->bitmapOffset; /* ptr to first byte of first word */
    /* containing a free bit */
    int bbn = index->bitmapOffset *
              8; /* first bit of word containing first free bit */
    byte *ep = index->bitmap +
                      ...
                      VnodeId vnode)
{
    *ec = 0;
    ...
    VLog(9, "VAllocBitmapEntry: volume = %x, vnode = %x", V_id(vp), vnode);
    ...
    byte *cbp =
        index->bitmap + (cbn / 8); /* ptr to byte containing requested bit */
    byte *bp = index->bitmap +
               index->bitmapOffset; /* ptr to first byte of first word */
    /* containing a free bit */
    byte *ep = index->bitmap +
...
void VUpdateVolume(Error *ec, Volume *vp)
{
    ...
    *ec = 0;
    ...
    if (*ec) {
        VLog(0, "VUpdateVolume/WriteVolumeHeader err: %d vol: %x (%s)", *ec,
             V_id(vp), V_name(vp));
        VForceOffline(vp);
    }
}
...
static void GetBitmap(Error *ec, Volume *vp, VnodeClass vclass)
{
    ...
    VLog(9, "Entering GetBitmap() for volume %x, vclass = %d", V_id(vp),
         vclass);
    *ec = 0;
    ...
    while ((bitNumber = vnext(vnode)) != -1) {
        if (vnode->vnodeMagic != vcp->magic) {
            VLog(
                0,
                "GetBitmap: addled vnode index in volume %s; volume needs salvage",
                V_name(vp));
    ...
    if (vp->nextVnodeUnique < unique) {
        VLog(
            0,
            "GetBitmap: bad volume uniquifier for volume %s; volume needs salvage",
            V_name(vp));
        *ec = VSALVAGE;
    }
}
...
#define Midnight(date) \
    ((date - TimeZoneCorrection) / OneDay * OneDay + TimeZoneCorrection)
...
static int
    nUpdatedVolumes; /* Updated with entry in UpdateList, salvage after crash flag on */
...
void VAddToVolumeUpdateList(Error *ec, Volume *vp)
{
    ...
    *ec            = 0;
    ...
    if (V_dontSalvage(vp) == 0) {
        VLog(29,
             "VAddToVolumeUpdateList: salvage was 0 - so not updating volume");
    ...
    if (*ec || camstatus) {
        VLog(29,
             "VAddToVolumeUpdateList: Error during Update Volume; returning");
        return;
    }
    if (!UpdateList) {
        ...
        if (nUpdatedVolumes == updateSize) {
            updateSize += UPDATE_LIST_SIZE;
            UpdateList = (VolumeId *)realloc((char *)UpdateList,
...
static void VScanUpdateList()
{
    ...
    for (i = gap = 0; i < nUpdatedVolumes; i++) {
        ...
        if (vp) {
            VLog(29, "ScanUpdateList: Going to Put volume %x", V_id(vp));
            VPutVolume(vp);
        }
        LWP_DispatchProcess();
    }
    nUpdatedVolumes -= gap;
    VLog(29, "ScanUpdateList: nUpdatedVolumes = %d", nUpdatedVolumes);
...
static void AddVolumeToHashTable(Volume *vp, int hashid)
{
    ...
    Volume *vptr;
    VLog(9, "Entering AddVolumeToHashTable for volume %x, hashid %u", V_id(vp),
         hashid);
...
void DeleteVolumeFromHashTable(Volume *vp)
{
    ...
    if (VolumeHashTable[hash] == vp) {
        VLog(29,
             "DeleteVolumeFromHashTable: Deleting volume %x from hash table",
             vp->hashid);
        ...
        VLog(29, "DeleteVolumeHashTable: Deleting volume %x from hashtable",
             vp->hashid);
    }
    vp->hashid = 0;
}
...
void VPrintCacheStats(FILE *fp)
{
    ...
    VLog(0,
         "Large vnode cache, %d entries, %d allocs, "
         "%d gets (%d reads), %d writes",
         vcp->cacheSize, vcp->allocs, vcp->gets, vcp->reads, vcp->writes);
    vcp = &VnodeClassInfo_Array[vSmall];
    VLog(0,
         "Small vnode cache,%d entries, %d allocs, "
         "%d gets (%d reads), %d writes",
         vcp->cacheSize, vcp->allocs, vcp->gets, vcp->reads, vcp->writes);
