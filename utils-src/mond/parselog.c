...
static bool PrintVmonSession()
{
    ...
    if (sum != 0)
        LogMsg(0, LogLevel, LogFile, "**** BAD SESSION RECORD ****");
    printf("%-12s:  ", "SESSION");
    PrintVenusId(&Venus);
    PrintHex(Session);
    PrintHex(Volume);
...
static bool PrintVmonCommEvent()
{
    ...
    if (sum != 0)
        LogMsg(0, LogLevel, LogFile, "**** BAD COMM RECORD ****");
...
static bool PrintClientCalls()
{
    ...
    if (sum != 0)
        LogMsg(0, LogLevel, LogFile, "**** BAD CLIENTCALL RECORD ****");
    printf("%-12s: ", "CLNTCALL");
    PrintVenusId(&Venus);
    PrintDecimal(Time);
    printf("\n");
...
static bool PrintClientRVM()
{
    ...
    if (sum != 0)
        LogMsg(0, LogLevel, LogFile, "**** BAD CLIENTRVM RECORD ****");
    printf("%-12s: ", "CLNTRVM");
    PrintVenusId(&Venus);
    PrintDecimal(Time);
    printf("\n");
...
static bool PrintVCB()
{
    ...
    if (sum != 0)
        LogMsg(0, LogLevel, LogFile, "**** BAD VCB RECORD ****");
...
static bool PrintAdviceCalls()
{
    ...
    int sum = ReadAdviceCall(&Venus, &Time, &User, &Stats, &Call_Size,
                             &Call_Stats, &Result_Size, &Result_Stats);

    if (sum != 0)
        LogMsg(0, LogLevel, LogFile, "**** BAD ADVICE RECORD ****");
...
static bool PrintMiniCache()
{
    ...
    int sum = ReadMiniCacheCall(&venus, &time, &vn_size, &vn_stat, &vfs_size,
                                &vfs_stat);

    if (sum != 0)
        LogMsg(0, LogLevel, LogFile, "**** BAD MINICACHE RECORD ****");
...
static bool PrintVmonOverflow()
{
    ...
    if (sum != 0)
        LogMsg(0, LogLevel, LogFile, "**** BAD OVERFLOW RECORD ****");
...
static bool PrintSrvCall()
{
    ...
    int sum = ReadSrvCall(&Vice, &Time, &CBSize, &CBCount, &ResSize, &ResCount,
                          &SmonSize, &SmonCount, &VolDSize, &VolDCount,
                          &MultiSize, &MultiCount, &Stats);

    if (sum != 0)
        LogMsg(0, LogLevel, LogFile, "**** BAD SRVCALL RECORD ****");
...
static bool PrintResEvent(void)
{
    ...
    if (sum != 0)
        LogMsg(0, LogLevel, LogFile, "**** BAD RESOLUTION RECORD ****");
...
static bool PrintSrvOvrflw()
{
    ...
    if (sum != 0)
        LogMsg(0, LogLevel, LogFile, "**** BAD SRVOVERFLOW RECORD ****");
...
static void PrintEventArray(VmonSessionEventArray *Events)
{
    for (int i = 0; i < nVSEs; i++) {
        ...
        if ((e->SuccessCount != 0) || (e->FailureCount != 0)) {
            printf("\t%3d  %5d  %10d  %10d  %5d\n", e->Opcode, e->SuccessCount,
                   e->SigmaT, e->SigmaTSquared, e->FailureCount);
        }
    }
}
...
static void PrintSessionStats(SessionStatistics *stats)
{
    printf("[%08d-%08d:%08d]  [%08d-%08d:%08d]\n", stats->EntriesStart,
           stats->EntriesEnd, stats->EntriesHighWater, stats->BytesStart,
...
static void PrintVCBStats(VCBStatistics *stats)
{
    ...
    PrintDecimal(stats->AcquireNoObjFails);
    printf("\n");
    printf("\t%-15s", "Validate");
    PrintDecimal(stats->Validates);
    PrintDecimal(stats->ValidateObjs);
    printf("\n");
    printf("\t%-15s", "FailedValidate");
    PrintDecimal(stats->FailedValidates);
    PrintDecimal(stats->FailedValidateObjs);
    printf("\n");
    printf("\t%-15s", "Break");
    PrintDecimal(stats->Breaks);
    PrintDecimal(stats->BreakObjs);
    PrintDecimal(stats->BreakVolOnly);
    PrintDecimal(stats->BreakRefs);
    printf("\n");
    printf("\t%-15s", "Clear");
    PrintDecimal(stats->Clears);
    PrintDecimal(stats->ClearObjs);
    PrintDecimal(stats->ClearRefs);
    printf("\n");
    printf("\t%-15s", "NoStamp");
    PrintDecimal(stats->NoStamp);
    PrintDecimal(stats->NoStampObjs);
    printf("\n");
}

static void PrintAdviceStats(AdviceStatistics *stats)
{
    PrintDecimal(stats->NotEnabled);
    printf("\n");
    PrintDecimal(stats->NotValid);
    printf("\n");
    PrintDecimal(stats->Outstanding);
    printf("\n");
    PrintDecimal(stats->ASRnotAllowed);
    printf("\n");
    PrintDecimal(stats->ASRinterval);
    printf("\n");
    PrintDecimal(stats->VolumeNull);
    printf("\n");
    PrintDecimal(stats->TotalNumberAttempts);
    printf("\n");
}
...
static void LogErrorPoint(int recordCounts[])
{
    ...
    LogMsg(10, LogLevel, LogFile, "\tClient MCalls:	%d",
           recordCounts[CLNTMCALL]);
    LogMsg(10, LogLevel, LogFile, "\tClient RVM Calls: %d",
           recordCounts[CLNTRVM]);
    LogMsg(10, LogLevel, LogFile, "\tVCB records:	%d", recordCounts[VCB]);
    LogMsg(10, LogLevel, LogFile, "\tAdvice records:	%d",
           recordCounts[ADVICE]);
    LogMsg(10, LogLevel, LogFile, "\tMinicache records:%d",
           recordCounts[MINICACHE]);
    ...
    LogMsg(10, LogLevel, LogFile, "\tResolve records:  %d",
           recordCounts[SRVRES]);
    LogMsg(10, LogLevel, LogFile, "\tRVM Res records:	%d",
           recordCounts[SRVRVMRES]);
...
static bool PrintIotInfoCall()
{
    ...
    if (sum != 0)
        LogMsg(0, LogLevel, LogFile, "**** BAD IOTINFO RECORD ****");
...
static bool PrintIotStatsCall()
{
    ...
    if (sum != 0)
        LogMsg(0, LogLevel, LogFile, "**** BAD IOTSTATS RECORD ****");
...
static bool PrintSubtreeStatsCall()
{
    ...
    if (sum != 0)
        LogMsg(0, LogLevel, LogFile, "**** BAD SUBTREE RECORD ****");
...
static bool PrintRepairStatsCall()
{
    ...
    if (sum != 0)
        LogMsg(0, LogLevel, LogFile, "**** BAD REPARI RECORD ****");
...
static bool PrintRwsStatsCall()
{
    ...
    if (sum != 0)
        LogMsg(0, LogLevel, LogFile, "**** BAD RWSSTATS RECORD ****");
...
{
    PrintDecimal(info->Tid);
    printf("\n");
    PrintDecimal(info->ResOpt);
    printf("\n");
    PrintDecimal(info->ElapsedTime);
    printf("\n");
    PrintDecimal(info->ReadSetSize);
    printf("\n");
    PrintDecimal(info->WriteSetSize);
    printf("\n");
    PrintDecimal(info->ReadVolNum);
    printf("\n");
    PrintDecimal(info->WriteVolNum);
    printf("\n");
    PrintDecimal(info->Validation);
    printf("\n");
    PrintDecimal(info->InvalidSize);
    printf("\n");
    PrintDecimal(info->BackupObjNum);
    printf("\n");
    PrintDecimal(info->LifeCycle);
    printf("\n");
    PrintDecimal(info->PredNum);
    printf("\n");
    PrintDecimal(info->SuccNum);
    printf("\n");
}
...
{
    PrintDecimal(stats->MaxElapsedTime);
    printf("\n");
    PrintDecimal(stats->AvgElapsedTime);
    printf("\n");
    PrintDecimal(stats->MaxReadSetSize);
    printf("\n");
    PrintDecimal(stats->AvgReadSetSize);
    printf("\n");
    PrintDecimal(stats->MaxWriteSetSize);
    printf("\n");
    PrintDecimal(stats->AvgWriteSetSize);
    printf("\n");
    PrintDecimal(stats->MaxReadVolNum);
    printf("\n");
    PrintDecimal(stats->AvgReadVolNum);
    printf("\n");
    PrintDecimal(stats->MaxWriteVolNum);
    printf("\n");
    PrintDecimal(stats->AvgWriteVolNum);
    printf("\n");
    PrintDecimal(stats->Committed);
    printf("\n");
    PrintDecimal(stats->Pending);
    printf("\n");
    PrintDecimal(stats->Resolved);
    printf("\n");
    PrintDecimal(stats->Repaired);
    printf("\n");
    PrintDecimal(stats->OCCRerun);
    printf("\n");
}

static void PrintSubtreeStats(LocalSubtreeStats *stats)
{
    PrintDecimal(stats->SubtreeNum);
    printf("\n");
    PrintDecimal(stats->MaxSubtreeSize);
    printf("\n");
    PrintDecimal(stats->AvgSubtreeSize);
    printf("\n");
    PrintDecimal(stats->MaxSubtreeHgt);
    printf("\n");
    PrintDecimal(stats->AvgSubtreeHgt);
    printf("\n");
    PrintDecimal(stats->MaxMutationNum);
    printf("\n");
    PrintDecimal(stats->AvgMutationNum);
    printf("\n");
}

static void PrintRepairStats(RepairSessionStats *stats)
{
    PrintDecimal(stats->SessionNum);
    printf("\n");
    PrintDecimal(stats->CommitNum);
    printf("\n");
    PrintDecimal(stats->AbortNum);
    printf("\n");
    PrintDecimal(stats->CheckNum);
    printf("\n");
    PrintDecimal(stats->PreserveNum);
    printf("\n");
    PrintDecimal(stats->DiscardNum);
    printf("\n");
    PrintDecimal(stats->RemoveNum);
    printf("\n");
    PrintDecimal(stats->GlobalViewNum);
    printf("\n");
    PrintDecimal(stats->LocalViewNum);
    printf("\n");
    PrintDecimal(stats->KeepLocalNum);
    printf("\n");
    PrintDecimal(stats->ListLocalNum);
    printf("\n");
    PrintDecimal(stats->NewCommand1Num);
    printf("\n");
    PrintDecimal(stats->NewCommand2Num);
    printf("\n");
    PrintDecimal(stats->NewCommand3Num);
    printf("\n");
    PrintDecimal(stats->NewCommand4Num);
    printf("\n");
    PrintDecimal(stats->NewCommand5Num);
    printf("\n");
    PrintDecimal(stats->NewCommand6Num);
    printf("\n");
    PrintDecimal(stats->NewCommand7Num);
    printf("\n");
    PrintDecimal(stats->NewCommand8Num);
    printf("\n");
    PrintDecimal(stats->RepMutationNum);
    printf("\n");
    PrintDecimal(stats->MissTargetNum);
    printf("\n");
    PrintDecimal(stats->MissParentNum);
    printf("\n");
    PrintDecimal(stats->AclDenyNum);
    printf("\n");
    PrintDecimal(stats->UpdateUpdateNum);
    printf("\n");
    PrintDecimal(stats->NameNameNum);
    printf("\n");
    PrintDecimal(stats->RemoveUpdateNum);
    printf("\n");
}

static void PrintRwsStats(ReadWriteSharingStats *stats)
{
    PrintDecimal(stats->Vid);
    printf("\n");
    PrintDecimal(stats->RwSharingCount);
    printf("\n");
    PrintDecimal(stats->DiscReadCount);
    printf("\n");
